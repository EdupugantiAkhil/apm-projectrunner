...
- [ ] Add `deregisterSource(name)` to `packages/web/src/api.ts` (`registerSource` at
      `api.ts:153` is the model; note this is *not* `removeWorktree` at `api.ts:157`).
- [ ] Render Remove for unmanaged sources at `packages/web/src/App.tsx:164`, which
      currently gates on `source.source.kind === 'managed'`.
- [ ] Use distinct confirmation copy per kind: managed says the directory is deleted,
      unmanaged says only the registration is forgotten and files are untouched.
- [ ] Test: unmanaged Remove calls `deregisterSource`, managed still calls
...
- [ ] Add `GET /api/v1/operations` returning durable operation records from SQLite,
      with query filters for deployment, instance, kind, and status, plus a cursor.
- [ ] Include a destructive-operation marker on each record (`cleanup`, `down`) so the
      timeline can flag them without the client hardcoding a kind list.
- [ ] Document the endpoint in `docs/control-plane-api.md` alongside the existing
      operations rows.
- [ ] Type the response in `packages/web/src/api.ts` and add a client method.
- [ ] Replace the session-only Operations view (`App.tsx:195-196`) with the durable list.
...
- [ ] Add profile endpoints: list discovered project-local and source-local profiles with
      ...
      imported profile.
- [ ] Mirror the TUI's trust semantics exactly — a source-local profile is untrusted
...
- [ ] Document the endpoints in `docs/control-plane-api.md`.
- [ ] Build a Profiles view: origin and trust badges, manifest review before import,
      re-import on change, remove-imported, and validate-against-checkout with the
      expansion report (`profiles.rs:427-485`).
- [ ] Do **not** build a profile editor. Blocked on the ops-layer mutation — see
      Sequencing constraints.
...
- [ ] Extend the device payload with an eligibility verdict and its reason, kept separate
      from SSH check status (`crates/switchyard-tui/src/tabs/devices.rs:194-236`).
- [ ] Include the implicit `local` device in the listing, or document that clients
      synthesize it — pick one and apply it consistently.
- [ ] Return the instances currently placed on a device so removal can be placement-aware
      (`devices.rs:66-102`).
- [ ] Widen the `DeviceStatus` type and render reachability and eligibility as two
      distinct columns in `DevicesView`.
- [ ] Show placed instances in the device removal dialog and block removal when non-empty.
- [ ] Surface authored and observed placement on instance cards
...
- [ ] Add "Add instance to existing deployment" as an entry point — today the builder
      only creates whole deployments.
- [ ] Checkout/worktree selector, filtered to registered sources.
- [ ] Trusted-profile selector, filtered to profiles valid for the chosen checkout
      (`crates/switchyard-tui/src/dialogs/wizard.rs:101-139`).
- [ ] Device selector restricted to eligible devices, showing the ineligibility reason
      inline for the rest.
- [ ] Render profile-defined parameters through `SchemaForm`, not free-text.
- [ ] Live expansion preview — services, ports, volumes — before the instance is
      appended (`wizard.rs:327-395`).
- [ ] Field-level errors for profile, device, and parameter problems, not one global
      validation banner.
...
- [ ] Derive consumers with consumed slots from the authored definition, including those
      with no binding (`crates/switchyard-tui/src/tabs/connections.rs:162-223`).
- [ ] Render unbound consumers as such rather than omitting them.
- [ ] Allow selecting a compatible complete group for an unbound consumer, reusing the
      existing compatibility filter and preview.
- [ ] Reuse the existing `ChangePreview` transition flow; first-bind has no old routes to
...
- [ ] Build a separate authored/desired-state connection view driven by the definition.
- [ ] Show it in place of the runtime patch bay when stopped, with copy that names which
      of the two the user is looking at.
- [ ] Allow editing desired connections offline; they take effect on the next `Up`.
- [ ] Keep the existing stopped-state callout and `Run Up` affordance.
...
- [ ] Type `RouteState.history` properly against the actual response.
- [ ] Show desired versus observed version separately — the active-routes table at
      `App.tsx:143-145` currently collapses them into one column.
- [ ] Add explicit transition state and previous version.
- [ ] Render rollback history (`crates/switchyard-tui/src/tabs/connections.rs:526-586`).
- [ ] Show a post-switch success/failure report after an apply.
...
- [ ] Add endpoints to list project run actions and run one, distinguishing structured
      from shell actions.
- [ ] Create/edit/delete of **structured** actions.
- [ ] Run an existing **shell** action, gated on the existing project-local
      acknowledgement (`run_scripts.rs:150`, `run_scripts.rs:154`).
- [ ] Confirmation preview before any execution, matching TUI behavior.

Explicitly out of scope for the web:

- [ ] ~~Create or edit shell actions from the browser~~ — CLI/TUI only. State this in the
      UI where a user would expect the button, so the absence reads as a decision.
...
- [ ] Free-text filter over deployment, instance, service, operation label, and output
      lines (`crates/switchyard-tui/src/tabs/operations.rs:337-365`).
- [ ] Instance and service log filtering in the event drawer, which today filters by
      deployment only (`App.tsx:127-130`).
- [ ] Destructive-operation markers in the timeline, from the Part 2 field.
- [ ] Keep the existing copy-as-plain-text action working against the filtered set.
...
- [ ] Startup profile per instance.
- [ ] Authored and observed device placement.
- [ ] Expanded service inventory grouped under the instance.
- [ ] Per-service state, health, and resource placement.
- [ ] The instance's active connections in one place.
- [ ] Instance-scoped recent operations.
- [ ] Fold in the existing node inspector in `DeploymentWorkspace.tsx` rather than
...
- [ ] Setup-progress checklist across source, profile, instance, startup, connection.
- [ ] Next-recommended-action affordance linking into the relevant view.
- [ ] Project-wide problem summary aggregating source, profile, deployment, device,
      operation, and connection problems (`home.rs:186-230`).
- [ ] Add Home to the top-level views at `App.tsx:94-121` and make it the default landing
...
- [ ] Attempt a non-interactive clone first, using ambient credential helpers and SSH
      agent, exactly as the TUI's underlying Git invocation does.
- [ ] On auth failure, prompt in-browser and feed the response to Git through a one-shot
...
- [ ] Treat unknown-host-key as an explicit UI approval step showing the fingerprint —
      not a passthrough TTY prompt.
- [ ] Stream clone progress as a normal operation so it appears in the timeline.
- [ ] Security-review this part specifically before merge; it is the only part that moves
      credential material through the browser.
