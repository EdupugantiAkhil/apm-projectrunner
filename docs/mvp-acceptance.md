...
| 21 | Duplicate a backend source when two UIs need different downstream groups. | **Automated planner validation and topology.** `crates/switchyard-planner/tests/planner.rs::one_instance_cannot_belong_to_two_groups` names both conflicting paths and advises a separate instance. The routing matrix proves separate instances can reuse one source and block across groups. |
...
| Inspect status/routes/source | `switchyard status FILE --routes`; `switchyard routes FILE` | `/api/v1/commands/status`, `/commands/routes`, `GET /deployments/{name}`, `GET /deployments/{name}/routes` | deployment rail, inspector, instance cards, membership view, active-routes table | Present; GUI tests cover deployment identity, routes, domains, and memberships. |
| Move instance to a group | `switchyard move FILE INSTANCE GROUP [--transition ...]` | `/api/v1/commands/membership` | membership view -> group -> complete preview -> transition -> **Apply membership move** | Present; Web tests cover the live move and stopped-definition editing. |
