...
APM ProjectRunner's durable project state lives at `.apmpr/state.sqlite3`. Generated
recovery manifests live below `.apmpr/generated/`, while runtime resources carry
APM ProjectRunner ownership and resource-hash labels. Stop the daemon before copying or
...
after a Docker Desktop restart, run `apmpr status` and then `apmpr up` to
...
1. Stop active control-plane work, then run `apmpr daemon stop`. Confirm
   `apmpr daemon status` reports no reachable daemon. Running application
   containers may remain up.
2. Replace the `apmpr`, `apmpr-daemon`, and `apmpr-router` binaries as one
   compatible set.
3. Re-run `apmpr daemon install .` to refresh the service definition to the current
   binary and start it. For foreground recovery, use `apmpr daemon run` instead.
   Opening the state store applies every pending embedded SQLite migration in order inside
   one transaction.
4. If the database already exists and has pending migrations, APM ProjectRunner first makes a
   consistent SQLite backup beside it. Its first name is
   `.apmpr/state.sqlite3.pre-migration-vN.bak`, where `N` is the old schema
   version. Existing backups are never overwritten; a numeric suffix is selected.
5. Inspect `apmpr daemon status`, deployment detail/status, route versions, and
   reconciliation diagnostics before applying changes.

`crates/apmpr-state/src/lib.rs::migrations_are_ordered_and_existing_database_is_backed_up`
...
`crates/apmpr-state/src/lib.rs::newer_schema_is_refused_without_a_backup`.
...
2. Preserve the rejected/current file for diagnosis:
   `mv .apmpr/state.sqlite3 .apmpr/state.sqlite3.after-upgrade`.
3. Copy, do not move, the appropriate backup into place:
   `cp .apmpr/state.sqlite3.pre-migration-vN.bak .apmpr/state.sqlite3`.
...
containers carry APM ProjectRunner ownership, deployment, resource-hash, and device labels,
...
- `crates/apmpr-daemon/tests/api.rs::restart_keeps_final_operation_state_in_sqlite`
- `crates/apmpr-daemon/tests/api.rs::applied_domains_bindings_and_deleted_database_recovery_survive_daemon_restart`
- `crates/apmpr-daemon/tests/api.rs::failed_live_binding_versions_and_rollback_history_survive_restart`

After restart, run `apmpr status <deployment> --routes` and verify the applied
...
- state-level manifest plus injected Docker-label recovery:
  `crates/apmpr-state/src/lib.rs::deleted_database_rebuilds_observed_state_without_inventing_applied_state`
- three-source comparison:
  `crates/apmpr-state/src/lib.rs::reconciliation_compares_all_three_sources_and_updates_observations`
...
Run `apmpr status <deployment> --routes` and inspect the GUI deployment detail.
...
`crates/apmpr-state/src/lib.rs::drift_codes_cover_missing_manifest_resources_hash_and_ownership`
and `reconciliation_compares_all_three_sources_and_updates_observations`;
`crates/apmpr-daemon/tests/api.rs::deployment_list_and_detail_include_applied_manifest_and_reconciliation`
proves API exposure.
...
native host gateway, rerun `apmpr up <deployment>`; host lifecycle recovery detects
the dead owned process and recreates it from generated configuration and current Docker
publications. Then check the stable UI hostname and `apmpr status --routes`.
...
`apmpr status <deployment> --routes`. If containers have not returned, use
`apmpr up <deployment>`; readiness waits for health and the host gateway refreshes
changed ephemeral publications. Do not use cleanup as a recovery step.

`examples/routing-matrix/smoke.sh` runs `docker compose restart`, then APM ProjectRunner
...
`apmpr down <deployment>` performs ownership checks and Compose down without
`--volumes`; named database volumes remain. `apmpr cleanup <deployment> --yes` is
...
- `crates/apmpr-cli/src/runtime.rs::down_does_not_delete_volumes`
- `crates/apmpr-cli/src/cli.rs::rejects_volume_deletion_through_down`
- `crates/apmpr-cli/src/runtime.rs::destructive_cleanup_requires_confirmation`
- `crates/apmpr-cli/src/runtime.rs::ownership_rejects_a_resource_without_managed_label`
