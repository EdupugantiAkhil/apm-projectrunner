...
import type { AdapterRecord, ApiClient, DeviceRecord, JsonSchema, Operation, ProfileDefinition, ProfileRecord, ProfileValidation, SourceRecord } from './api'
...
export default function DeploymentBuilder({ client, sources, profiles = [], devices = [], deployment, close, saved, onOperation, report }: { client: ApiClient; sources: SourceRecord[]; profiles?: ProfileRecord[]; devices?: DeviceRecord[]; deployment?: string; close: () => void; saved: (name: string) => Promise<void>; onOperation: (operation: Operation) => void; report: (error: unknown) => void }) {
  return deployment
    ...
    : <NewDeploymentBuilder client={client} sources={sources} close={close} saved={saved} onOperation={onOperation} report={report} />
}
...
function NewDeploymentBuilder({ client, sources, close, saved, onOperation, report }: { client: ApiClient; sources: SourceRecord[]; close: () => void; saved: (name: string) => Promise<void>; onOperation: (operation: Operation) => void; report: (error: unknown) => void }) {
