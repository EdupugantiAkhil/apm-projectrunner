...
describe('Switchyard GUI', () => {
  ...
  it('shows one honest per-instance inspector with placement, services, connections, and deployment-scoped operations', async () => {
    const user = userEvent.setup(); const client = new ApiClient('test'); const inspected = { ...deployment, snapshot: { spec: { ...deployment.snapshot.spec, instances: deployment.snapshot.spec.instances.map((instance) => instance.name === 'ui-feature' ? { ...instance, block: 'web-ui', source: 'feature-ui' } : instance), blocks: { 'web-ui': { services: { web: { consumes: { java: { protocol: 'tcp' }, python: { protocol: 'grpc' }, database: { protocol: 'tcp' } } }, worker: {} } } } } }, resources: [{ kind: 'container', id: 'web', name: 'comparison-ui-feature-web', labels: {}, state: 'running (healthy)', device: 'build-host' }, { kind: 'container', id: 'worker', name: 'comparison-ui-feature-worker', labels: {}, state: 'running (unhealthy)', device: 'local' }] }
    vi.spyOn(client, 'deployment').mockResolvedValue(inspected); vi.spyOn(client, 'operations').mockResolvedValue({ apiVersion: 'v1', operations: [{ apiVersion: 'v1', id: 'op-ui-feature', deployment: 'comparison', kind: 'status', destructive: false, status: 'succeeded', startedAt: 8, finishedAt: 9, error: null, result: { exitCode: 0, stdout: 'ui-feature is healthy', stderr: '' } }, { apiVersion: 'v1', id: 'op-other', deployment: 'comparison', kind: 'status', destructive: false, status: 'succeeded', startedAt: 6, finishedAt: 7, error: null, result: { exitCode: 0, stdout: 'backend-b is healthy', stderr: '' } }, { apiVersion: 'v1', id: 'op-staging', deployment: 'staging', kind: 'status', destructive: false, status: 'succeeded', startedAt: 4, finishedAt: 5, error: null, result: null }], nextCursor: null })
    ...
    const inspector = screen.getByRole('complementary', { name: 'Inspector' }); expect(within(inspector).getByRole('heading', { name: 'ui-feature' })).toBeInTheDocument(); expect(within(inspector).getByText('unavailable — the applied snapshot records only the expanded block')).toBeInTheDocument(); expect(within(inspector).getByText('web-ui')).toBeInTheDocument(); expect(within(inspector).getByText('feature-ui')).toBeInTheDocument(); expect(within(inspector).getByText('build-host, local')).toBeInTheDocument()
    for (const service of ['web', 'worker']) { const row = within(inspector).getByText(service).closest('li')!; expect(within(row).getByText('not reported per service')).toBeInTheDocument(); expect(within(row).getAllByText('not observed per service')).toHaveLength(2) }
    ...
    expect(within(inspector).getByText('op-ui-feature')).toBeInTheDocument(); expect(within(inspector).getByText('op-other')).toBeInTheDocument(); expect(within(inspector).queryByText('op-staging')).not.toBeInTheDocument(); expect(within(inspector).getByText(/Instance scoping is unavailable/)).toBeInTheDocument(); expect(within(inspector).getByText(/no rejected instance filter or output-text inference is used/)).toBeInTheDocument()
  })
