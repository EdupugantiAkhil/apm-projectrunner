...
describe('Switchyard GUI', () => {
  ...
  it('shows one honest per-instance inspector with placement, services, connections, and deployment-scoped operations', async () => {
    const user = userEvent.setup(); const client = new ApiClient('test'); const inspected = { ...deployment, snapshot: { spec: { ...deployment.snapshot.spec, instances: deployment.snapshot.spec.instances.map((instance) => instance.name === 'ui-feature' ? { ...instance, block: 'web-ui', source: 'feature-ui' } : instance), blocks: { 'web-ui': { services: { web: { consumes: { java: { protocol: 'tcp' }, python: { protocol: 'grpc' }, database: { protocol: 'tcp' } } }, worker: {} } } } } }, resources: [{ kind: 'container', id: 'web', name: 'opaque-runtime-name', labels: { 'dev.switchyard.instance': 'ui-feature', 'dev.switchyard.service': 'web' }, state: 'running (healthy)', device: 'build-host' }, { kind: 'container', id: 'worker', name: 'comparison-ui-feature-worker', labels: {} as Record<string, string>, state: 'running (unhealthy)', device: 'local' }] }
