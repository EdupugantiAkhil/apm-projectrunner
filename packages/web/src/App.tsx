...
const stoppedDiagnostic = (detail: DeploymentDetail) => detail.resources.length === 0
  ...
  : undefined
const instanceResources = (detail: DeploymentDetail, instance: string) => detail.resources.filter((resource) => resource.labels['dev.apmpr.instance'] === instance)
const serviceResources = (detail: DeploymentDetail, instance: string, service: string) => instanceResources(detail, instance).filter((resource) => resource.labels['dev.apmpr.service'] === service)
...
export default function App({ client = defaultClient }: { client?: ApiClient }) {
  ...
  const runCommand = async (kind: 'validate' | 'plan' | 'status' | 'logs' | 'open' | 'apply' | 'down' | 'cleanup', target?: string) => {
    ...
    const bundle = `.apmpr/generated/${selected}/resolved-deployment.yaml`
  ...
  return <div className="app-shell">
    <aside className="rail" aria-label="Deployment rail">
      <div className="brand">APMPR <span>LOCAL</span><small title={project?.root}>{project?.name ?? 'Loading project…'}</small></div>
...
function DevicesView({ client, devices, loading, reload, report }: { client: ApiClient; devices: DeviceRecord[]; loading: boolean; reload: () => Promise<void>; report: (error: unknown) => void }) {
  ...
  return <section><h1>Devices</h1><p className="muted">Reachability reports the SSH path. Eligibility separately reports whether APM ProjectRunner can execute containers there.</p>
