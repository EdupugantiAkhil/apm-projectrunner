...
const stoppedDiagnostic = (detail: DeploymentDetail) => detail.resources.length === 0
  ...
  : undefined
const instanceResources = (detail: DeploymentDetail, instance: string) => detail.resources.filter((resource) => resource.labels['dev.switchyard.instance'] === instance || resource.name.includes(instance))
...
function DeploymentInspector({ client, detail, routes, operations, selectedInstance, onOperation, refresh, report }: { client: ApiClient; detail: DeploymentDetail; routes: RouteState | null; operations: Operation[]; selectedInstance: string; onOperation: (operation: Operation) => Promise<Operation | null>; refresh: () => Promise<void>; report: (error: unknown) => void }) {
  ...
  if (selectedInstance && instance) {
    ...
    return <section className="instance-inspector" aria-label={`Selected instance ${selectedInstance}`}><p className="eyebrow">Instance</p><h3>{selectedInstance}</h3><dl><dt>Startup profile</dt><dd>unavailable — the applied snapshot records only the expanded block</dd><dt>Expanded block</dt><dd className="mono">{instance.block ?? 'not recorded'}</dd><dt>Authored placement</dt><dd className="mono">{instance.device ?? 'local'}</dd><dt>Observed placement</dt><dd className="mono">{observedDevices.length ? observedDevices.join(', ') : 'not observed'}</dd><dt>Source</dt><dd className="mono">{instance.source ?? 'not recorded'}</dd>{identity && <><dt>Path</dt><dd className="mono">{identity.path}</dd><dt>Ref</dt><dd className="mono">{identity.ref ?? 'detached'}</dd><dt>Commit</dt><dd className="mono">{short(identity.commit)}</dd></>}</dl>
      <h3>Expanded services</h3><p className="help">Service names come from the applied snapshot. The deployment detail response does not associate observed resources with individual services, so per-service state, health, and placement are unavailable.</p>{services.length ? <ul className="inspector-services">{services.map((service) => <li key={service}><strong>{service}</strong><dl><dt>State</dt><dd>not observed per service</dd><dt>Health</dt><dd>not reported per service</dd><dt>Resource placement</dt><dd>not observed per service</dd></dl></li>)}</ul> : <p className="muted">No services are declared by this expanded block.</p>}
