...
describe('ApiClient', () => {
  ...
  it('uses versioned profile selection, review, validation, import, and removal contracts', async () => {
    ...
    await client.profiles(); await client.profile(profile.name, profile.deployment, origin); await client.profileManifest(profile.name, origin.source); await client.validateProfile(profile, 'checkout one', { targetDeployment: 'target app', instanceName: 'worker', device: 'local', parameters: { MODE: 'safe' } }); await client.importProfile(profile.name, origin.source, 'review/hash')
    ...
    expect(fetchMock.mock.calls[3][1].body).toBe(JSON.stringify({ deployment: 'demo app', origin, checkout: 'checkout one', targetDeployment: 'target app', instanceName: 'worker', device: 'local', parameters: { MODE: 'safe' } }))
