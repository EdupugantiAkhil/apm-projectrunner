...
use apmpr_adapter_sdk::{AdapterKind, Diagnostic as AdapterDiagnostic, SourceIdentity};
use apmpr_adapters::built_in_registry;
use apmpr_sources::SourceManager;
use serde::{Deserialize, Serialize};
use serde_json::{Value, json};
use sha2::{Digest, Sha256};

const INSTANCE_LABEL: &str = "dev.apmpr.instance";
const SERVICE_LABEL: &str = "dev.apmpr.service";
/// API group used before the rename to APM ProjectRunner. Documents still carrying it are
/// rejected with the replacement prefix rather than an opaque deserialization error.
const RENAMED_API_GROUP: &str = "switchyard.dev/";
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MigrationRequired(version) => write!(
                f,
                "deployment uses apiVersion {version}; run `apmpr migrate` to update it to {API_VERSION}"
            ),
            Self::RenamedApiGroup(version) => write!(
                f,
                "deployment uses apiVersion {version}; Switchyard was renamed to APM ProjectRunner, so replace the `{RENAMED_API_GROUP}` prefix with `apmpr.dev/` (this document becomes {API_VERSION})"
            ),
...
pub struct ManagedProfilePlan {
    ...
    /// compatibility within `apmpr.dev/managed-profile/v1alpha1`; the name is
...
pub fn load_bundle_from_str(input: &str, path: &Path) -> Result<Bundle, PlannerError> {
    ...
    if version.api_version.starts_with(RENAMED_API_GROUP) {
        return Err(PlannerError::RenamedApiGroup(version.api_version));
    }
    if version.api_version == "apmpr.dev/v1alpha1" {
...
/// Atomically writes disposable artifacts beneath `.apmpr/generated/<deployment>`.
...
fn validate(
    ...
    let generated_definition = bundle
        ...
        && bundle
            .definition_dir
            .components()
            .any(|component| component.as_os_str() == ".apmpr");
...
fn validate_execution(
    ...
    service_name: &str,
    service: &Service,
    adapters: &apmpr_adapter_sdk::AdapterRegistry,
...
fn generate(
    ...
    for instance in &bundle.spec.instances {
        ...
        if let Some(device) = remote_device {
            instance_labels.insert("dev.apmpr.device".into(), device.to_owned());
        }
        let block = &bundle.spec.blocks[&instance.block];
        ...
        if routed {
            ...
            let admin_socket = PathBuf::from("/tmp/apmpr-admin.socket");
    ...
    for (device_name, services) in remote_services {
        ...
        remote_labels.insert("dev.apmpr.device".into(), device_name.clone());
...
fn managed_profiles(bundle: &Bundle) -> BTreeMap<String, ManagedProfilePlan> {
    ...
    for (instance, profile) in &bundle.spec.managed_profiles {
        ...
        result.insert(
            instance.clone(),
            ManagedProfilePlan {
                api_version: "apmpr.dev/managed-profile/v1alpha1".into(),
...
fn add_runtime_fields(
    ...
    variables.insert("APMPR_DEPLOYMENT".into(), bundle.metadata.name.clone());
    variables.insert("APMPR_INSTANCE".into(), instance.name.clone());
...
fn overlay_secret_variable(instance: &str, key: &str) -> String {
    ...
    format!("APMPR_OVERLAY_SECRET_{}", digest[..16].to_ascii_uppercase())
}
...
fn compose_sidecar(
    ...
    let mut sidecar = json!({
        "image": image,
        "user": "${APMPR_UID:-1000}:${APMPR_GID:-1000}",
        "restart": "unless-stopped",
        "network_mode": format!("service:{namespace_service}"),
        "command": [
            "/usr/local/bin/apmpr-router",
            format!("/config/{}", config_path.file_name().unwrap_or_default().to_string_lossy()),
            "/tmp/apmpr-admin.socket",
        ],
        "environment": {
            "APMPR_ROUTER_TOKEN": "${APMPR_ROUTER_TOKEN:?set APMPR_ROUTER_TOKEN}"
        },
        "volumes": [
        ...
        "healthcheck": {
            "test": ["CMD", "test", "-S", "/tmp/apmpr-admin.socket"],
            "interval": "1s",
            "timeout": "1s",
            "retries": 30,
        },
...
fn ownership_labels(deployment: &str, resource_hash: &str) -> BTreeMap<String, String> {
    BTreeMap::from([
        ("dev.apmpr.deployment".into(), deployment.into()),
        ("dev.apmpr.managed".into(), "true".into()),
        ("dev.apmpr.resource-hash".into(), resource_hash.into()),
    ])
}
