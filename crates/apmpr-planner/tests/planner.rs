...
fn bundle() -> apmpr_planner::Bundle {
...
fn v1alpha1_loader_error_names_the_migration_command() {
    ...
    .unwrap_err()
    .to_string();
    assert!(error.contains("run `apmpr migrate`"));
    assert!(error.contains("apmpr.dev/v1alpha2"));
}

#[test]
fn pre_rename_api_group_error_names_the_replacement_prefix() {
    let directory = tempfile::tempdir().unwrap();
    // Both pre-rename spellings must be recognized, not just the one matching the current
    // schema version, or a v1alpha1-era file reports an opaque deserialization failure.
    for version in ["switchyard.dev/v1alpha2", "switchyard.dev/v1alpha1"] {
        let error = load_bundle_from_str(
            &format!(
                "apiVersion: {version}\nkind: Deployment\nmetadata: {{ name: demo }}\nspec: {{}}\n"
            ),
        ...
        assert!(error.contains("switchyard.dev/"), "{error}");
        assert!(error.contains("apmpr.dev/"), "{error}");
        assert!(error.contains("apmpr.dev/v1alpha2"), "{error}");
    }
}
...
fn remote_provider_is_partitioned_and_routed_by_device_host() {
    ...
    assert_eq!(
        remote_compose["networks"][network]["labels"]["dev.apmpr.deployment"],
        "comparison"
    );
    assert_eq!(
        remote_compose["networks"][network]["labels"]["dev.apmpr.device"],
        "builder"
    );
    assert_eq!(
        remote_compose["networks"][network]["labels"]["dev.apmpr.resource-hash"],
    ...
    assert_eq!(
        remote_compose["services"][service]["labels"]["dev.apmpr.service"],
        "api"
    );
    let volume = &remote_compose["volumes"]["comparison--provider-main--data"];
    assert_eq!(volume["labels"]["dev.apmpr.managed"], "true");
    assert_eq!(volume["labels"]["dev.apmpr.deployment"], "comparison");
    assert_eq!(volume["labels"]["dev.apmpr.device"], "builder");
    assert_eq!(volume["labels"]["dev.apmpr.instance"], "provider-main");
    assert_eq!(volume["labels"]["dev.apmpr.service"], "api");
    assert_eq!(
        volume["labels"]["dev.apmpr.resource-hash"],
        generated.resource_hash
    );
    assert!(
        !generated
...
"#,
    ...
    assert!(first.compose_yaml.contains("APMPR_OVERLAY_SECRET_"));
...
"#,
    ...
    if let apmpr_planner::Execution::Container { image, .. } = &mut rebuilt_bundle
        .spec
        .blocks
        .get_mut("provider")
        .unwrap()
...
fn compose_and_manifest_are_deterministic_and_owned() {
    ...
    assert_eq!(first.artifact_dir, Path::new(".apmpr/generated/comparison"));
    ...
    assert!(first.compose_yaml.contains("dev.apmpr.resource-hash"));
    ...
    for (service, expected_component) in [
        ...
        assert_eq!(
            compose["services"][service]["labels"]["dev.apmpr.instance"],
        ...
        assert_eq!(
            compose["services"][service]["labels"]["dev.apmpr.service"],
            expected_component
        );
    }
    assert_eq!(
...
fn bindings_and_direct_routes_are_not_authored_schema_fields() {
    ...
    for field in ["bindings", "routes"] {
        ...
        let error = apmpr_planner::load_bundle_from_str(
            &encoded,
...
fn writes_deterministic_credential_free_managed_profile_metadata() {
    ...
    // `apmpr.dev/managed-profile/v1alpha1` spells the owning instance `ui`. The Rust
...
fn missing_source_paths_plan_without_writing_and_are_created_by_up() {
    ...
    assert!(!Path::new(".apmpr/generated/comparison").exists());
}
...
fn fixture_file_does_not_need_generated_state() {
    ...
    assert!(!yaml.contains(".apmpr/generated"));
}
...
fn external_bundle(instances: &str, members: &str) -> apmpr_planner::Bundle {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("deployment.yaml");
    let yaml = format!(
        r#"apiVersion: apmpr.dev/v1alpha2
kind: Deployment
