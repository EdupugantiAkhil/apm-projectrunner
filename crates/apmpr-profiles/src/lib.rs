//! Startup-profile discovery, trust, import, and projection for APM ProjectRunner clients.
...
use apmpr_planner::{Block, Execution};
use apmpr_sources::{RegisteredSourceInspection, SourceManager};
use apmpr_state::{ImportedProfile, StateError, StateStore};
use serde::{Deserialize, Serialize};
use serde_json::Value;
use sha2::{Digest, Sha256};

const MANIFEST_FILE: &str = "apmpr-profiles.yaml";
...
/// Verbatim source manifest and digest presented at an explicit trust boundary.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct SourceProfileManifestReview {
    pub manifest: String,
...
pub enum ProfileError {
    ...
    ManifestReviewChanged { source: String },
    Definition(String),
    State(StateError),
    Clock,
}
...
impl ProfileError {
    pub const fn code(&self) -> &'static str {
        match self {
            ...
            Self::ManifestReviewChanged { .. } => "profile_manifest_review_changed",
...
impl fmt::Display for ProfileError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ManifestReviewChanged { source } => write!(
                formatter,
                "source `{source}` profile manifest changed after it was reviewed"
            ),
...
pub fn discover_source_profiles(
    ...
    parse_source_profile_manifest(&path, &contents).map(Some)
}

fn parse_source_profile_manifest(
    path: &Path,
    contents: &str,
) -> Result<SourceProfileManifest, ProfileError> {
    let raw: RawManifest =
    ...
    for (name, value) in raw.profiles {
        ...
        apmpr_planner::validate_block(&name, &block).map_err(|diagnostics| {
            ProfileError::InvalidProfile {
                profile: name.clone(),
                message: diagnostics
                    .iter()
...
pub fn list_profiles(
    ...
    let bundle = apmpr_planner::load_bundle(deployment_definition)
        .map_err(|error| ProfileError::Definition(error.to_string()))?;
    let store = StateStore::open(project_dir.join(".apmpr/state.sqlite3"))?.0;
...
/// Reads the verbatim manifest that an interactive client must present before import.
pub fn review_source_profile_manifest(
    project_dir: &Path,
    source_name: &str,
) -> Result<SourceProfileManifestReview, ProfileError> {
    let store = StateStore::open(project_dir.join(".apmpr/state.sqlite3"))?.0;
    let source = store
        .source(source_name)?
        .ok_or_else(|| ProfileError::SourceNotFound {
    ...
    let path = source.path.join(MANIFEST_FILE);
    let manifest = fs::read_to_string(&path).map_err(|source| ProfileError::Io { path, source })?;
    Ok(SourceProfileManifestReview {
        review_hash: format!("{:x}", Sha256::digest(manifest.as_bytes())),
        manifest,
    })
}

/// Imports exactly the source manifest content previously reviewed by the caller.
pub fn import_reviewed_source_profile(
    project_dir: &Path,
    source_name: &str,
    profile_name: &str,
    ...
    let current_hash = format!("{:x}", Sha256::digest(contents.as_bytes()));
    if current_hash != reviewed_manifest_hash {
        return Err(ProfileError::ManifestReviewChanged {
            source: source_name.into(),
        });
    }
    let manifest = parse_source_profile_manifest(&path, &contents)?;
    let block =
        manifest
            .profiles
            .get(profile_name)
...
pub fn import_source_profile(
    ...
    apmpr_planner::validate_block(profile_name, block).map_err(|diagnostics| {
        ProfileError::InvalidProfile {
            profile: profile_name.into(),
            message: diagnostics
                .iter()
...
mod tests {
    use super::*;
    use apmpr_state::{RegisteredSource, RegisteredSourceKind};
...
"#;
    ...
    fn register_named_source(project: &Path, name: &str, source_root: &Path) {
        let store = StateStore::open(project.join(".apmpr/state.sqlite3"))
            .unwrap()
            .0;
        store
...
"#,
    ...
    #[test]
    fn reviewed_import_rejects_manifest_changes_before_recording_trust() {
        let project = TempDir::new().unwrap();
        let source = project.path().join("source");
        write_manifest(&source, VALID_MANIFEST);
        register_source(project.path(), &source);

        let review = review_source_profile_manifest(project.path(), "checkout").unwrap();
        write_manifest(
            &source,
            &VALID_MANIFEST.replace("busybox:latest", "busybox:stable"),
        );
        assert!(matches!(
            import_reviewed_source_profile(
                project.path(),
                "checkout",
                "api",
                &review.review_hash,
            ...
            .unwrap_err(),
            ProfileError::ManifestReviewChanged { source } if source == "checkout"
        ));
        let store = StateStore::open(project.path().join(".apmpr/state.sqlite3"))
            .unwrap()
            .0;
        ...
        let review = review_source_profile_manifest(project.path(), "checkout").unwrap();
        let imported =
            import_reviewed_source_profile(project.path(), "checkout", "api", &review.review_hash)
                .unwrap();
