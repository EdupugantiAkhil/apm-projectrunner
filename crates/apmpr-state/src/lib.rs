//! Persistent, synchronous SQLite state for the APM ProjectRunner control plane.
//!
//! The intended project-local database is `.apmpr/state.sqlite3`, but callers
...
//! labels are reduced to the six trusted APM ProjectRunner ownership fields before storage.
//!
...
pub const INSTANCE_LABEL: &str = "dev.apmpr.instance";
/// Service ownership label used by the existing Docker runtime.
pub const SERVICE_LABEL: &str = "dev.apmpr.service";
/// Device placement label used by the existing Docker runtime.
pub const DEVICE_LABEL: &str = "dev.apmpr.device";
/// Project-local marker created when an existing folder is adopted by APM ProjectRunner.
pub const PROJECT_MARKER_PATH: &str = ".apmpr/project.json";
/// Contract version stored inside [`PROJECT_MARKER_PATH`].
pub const PROJECT_API_VERSION: &str = "apmpr.dev/v1alpha1";

/// Stable, human-facing identity for an APM ProjectRunner project folder.
...
pub enum RegisteredSourceKind {
    ...
    Unmanaged,
    /// A clone or worktree created under APM ProjectRunner's managed roots.
    Managed,
}
        ...
        connection
            .execute(
                ...
                params![
                    ...
                    "running",
                    format!(r#"{{"dev.apmpr.deployment":"demo-v{version}","dev.apmpr.managed":"true"}}"#),
                    5_000 + version
                ],
            )
            .unwrap();
