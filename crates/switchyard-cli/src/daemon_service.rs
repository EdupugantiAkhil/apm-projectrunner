...
fn prepare_log(project_root: &Path) -> Result<PathBuf, ServiceError> {
    let state = project_root.join(".switchyard");
...
fn install_systemd(
    ...
    let unit_name = format!("switchyard-{}.service", service_key(project_root));
...
fn install_launchd(
    ...
    let label = format!("dev.switchyard.daemon.{}", service_key(project_root));
    let definition = home
...
fn render_systemd(
    ...
    Ok(format!(
        "[Unit]\nDescription=Switchyard daemon for {project}\n\n[Service]\nType=simple\nWorkingDirectory={project}\nExecStart={executable} daemon run\nEnvironment={path}\nRestart=on-failure\nRestartSec=2\nStandardOutput={log}\nStandardError={log}\n\n[Install]\nWantedBy=default.target\n"
    ))
}
...
mod tests {
    ...
    fn systemd_unit_quotes_paths_and_percent_specifiers() {
        let unit = render_systemd(
            ...
            Path::new("/opt/Switch Yard/bin/switchyard"),
            Path::new("/tmp/My Project%20/.switchyard/daemon.log"),
            "/opt/Switch Yard/bin:/usr/bin",
        )
        .unwrap();
        assert!(unit.contains("WorkingDirectory=\"/tmp/My Project%%20\""));
        assert!(unit.contains("ExecStart=\"/opt/Switch Yard/bin/switchyard\" daemon run"));
        assert!(unit.contains("Restart=on-failure"));
        assert!(unit.contains("Environment=\"PATH=/opt/Switch Yard/bin:/usr/bin\""));
        assert!(
            unit.contains("StandardError=\"append:/tmp/My Project%%20/.switchyard/daemon.log\"")
        );
    }
    ...
    fn launchd_plist_escapes_paths_and_has_service_posture() {
        let plist = render_launchd(
            "dev.switchyard.daemon.demo",
            Path::new("/tmp/A&B"),
            Path::new("/opt/<switchyard>"),
            Path::new("/tmp/A&B/.switchyard/daemon.log"),
            "/opt/A&B/bin:/usr/bin",
        )
        .unwrap();
        assert!(plist.contains("<string>/tmp/A&amp;B</string>"));
        assert!(plist.contains("<string>/opt/&lt;switchyard&gt;</string>"));
