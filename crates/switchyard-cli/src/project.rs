...
pub enum ProjectError {
    Io(io::Error),
    Message(String),
    State(switchyard_state::StateError),
    Source(switchyard_sources::SourceError),
}
...
impl From<switchyard_state::StateError> for ProjectError {
    fn from(value: switchyard_state::StateError) -> Self {
        Self::State(value)
    }
}
impl From<switchyard_sources::SourceError> for ProjectError {
    fn from(value: switchyard_sources::SourceError) -> Self {
        Self::Source(value)
    }
}
...
pub fn register(
    ...
    if let Some(existing) = &existing {
        if existing != &metadata {
            return Err(ProjectError::Message(format!(
                "folder is already registered as Switchyard project `{}`",
                existing.name
            )));
        }
    }

    fs::create_dir_all(root.join(".switchyard"))?;
    let (store, _) = StateStore::open(root.join(".switchyard/state.sqlite3"))?;
