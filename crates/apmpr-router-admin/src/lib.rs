//! Typed synchronous client for the local APM ProjectRunner router administration channel.
...
const MANAGED_LABEL: &str = "dev.apmpr.managed";
const DEPLOYMENT_LABEL: &str = "dev.apmpr.deployment";
const INSTANCE_LABEL: &str = "dev.apmpr.instance";
const RESOURCE_HASH_LABEL: &str = "dev.apmpr.resource-hash";
...
pub enum AdminEndpoint {
    ...
    Unix(PathBuf),
    /// An owner-only socket inside an exact APM ProjectRunner-owned Docker container.
    DockerExec {
        container: String,
        socket_path: PathBuf,
        deployment: String,
