---
name: apmpr-project
description: Inspect, author, validate, and safely operate an APM ProjectRunner development topology. Use for deployment.yaml, overlays, startup profiles, sources, devices, instances, service groups, bindings, plans, lifecycle commands, or diagnostics in this project.
---

# APM ProjectRunner Project

Reach the same valid desired state as the TUI by editing authored configuration and
using APM ProjectRunner's validator and planner. Treat diagnostics as authoritative.
...
2. Run `apmpr source list` to identify registered repositories and exact
   checkouts. Do not infer registry state from paths alone.
3. Run `apmpr device list` to identify registered SSH devices and their last
   eligibility result.
4. Read `.apmpr/generated/<deployment-name>/` only when comparing the last plan or
   investigating drift. It is derived evidence, never an authoring surface.
5. Run `apmpr status deployment.yaml` to compare desired, applied, and observed
   state. Add `--routes` when investigating bindings.

Never edit `.apmpr/generated/` or `.apmpr/state.sqlite3`. Do not hand-edit
...
`.apmpr/clones/`; `clone:` adopts existing Git storage. Then declare each runnable
...
Only source worktrees are edited, mounted, or run. `apmpr up` creates missing
...
`apmpr-profiles.yaml` at its checkout root. The format is `version: 1`; each key
...
an error. Use `apmpr tui .` when explicit profile review or guided authoring is
needed.
...
- Use `apmpr down deployment.yaml` for normal stopping; it preserves volumes.
- Run `apmpr cleanup deployment.yaml --yes` only after explicit user intent to
