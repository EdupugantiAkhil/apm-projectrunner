...
pub fn write_bundle(
    ...
    let authored = apmpr_planner::load_bundle(deployment_path)?;
    let deployment = authored.metadata.name.clone();
    let default_deployment_name = safe_file_component(&deployment);
    let devices = apmpr_state::StateStore::open(workspace_root.join(".apmpr/state.sqlite3"))
        .ok()
        ...
        .collect();
    let planned = apmpr_planner::plan_with_devices(&authored, &devices);
    ...
    let daemon_status = apmpr_daemon::client::daemon_status(workspace_root)
        .ok()
        .flatten();
    let state = match daemon_status {
        Some(status) => {
            let detail = match apmpr_daemon::client::deployment_detail(workspace_root, &deployment)
            {
        ...
        None => json!({
            ...
            "generated": collect_json_files(
                &workspace_root.join(".apmpr/generated").join(&deployment),
            ),
            "runtime": collect_json_files(
                &workspace_root.join(".apmpr/run").join(&deployment),
            ),
        }),
    };
    ...
    let run_dir = workspace_root.join(".apmpr/run").join(&deployment);
    let mut report = json!({
        "schemaVersion": "apmpr.dev/diagnostics/v1alpha1",
        "createdAtUnixSeconds": now_seconds(),
        "tool": {
            "version": env!("CARGO_PKG_VERSION"),
            "gitDescribe": env!("APMPR_GIT_DESCRIBE"),
        },
        "system": {
            "os": env::consts::OS,
...
fn collect_router_events(run_dir: &Path, plan: Option<&apmpr_planner::Plan>) -> Value {
    let Some(token) = env::var_os("APMPR_ROUTER_TOKEN") else {
        return json!({"unavailable": "APMPR_ROUTER_TOKEN is not set"});
    };
    let Some(token) = token.to_str() else {
        return json!({"unavailable": "APMPR_ROUTER_TOKEN is not valid UTF-8"});
    };
    let mut endpoints = BTreeMap::from([(
        "host".to_owned(),
        apmpr_router_admin::AdminEndpoint::unix(run_dir.join("host.socket")),
    )]);
    if let Some(plan) = plan {
        for (name, sidecar) in &plan.sidecars {
            endpoints.insert(
                name.clone(),
                apmpr_router_admin::AdminEndpoint::docker_exec(
                    &sidecar.service,
    ...
    Value::Object(
        endpoints
            .into_iter()
            .map(|(name, endpoint)| {
                let value = apmpr_router_admin::events(&endpoint, token)
...
impl Redactor {
    fn from_process(workspace_root: &Path) -> Self {
        ...
        let mut environment_values = env::vars_os()
            .filter_map(|(name, value)| {
                ...
                apmpr_planner::credential_like_key(&name).then_some(value.into_string().ok()?)
            })
            .filter(|value| !value.is_empty())
            .collect::<Vec<_>>();
        if let Ok(contents) = fs::read_to_string(workspace_root.join(".apmpr/daemon.json")) {
    ...
    fn redact_object(&self, object: &mut Map<String, Value>) {
        for (key, value) in object {
            if apmpr_planner::credential_like_key(key) {
    ...
    fn redact_string(&self, string: &mut String) {
        if apmpr_planner::redact_event_line(string) == REDACTED {
