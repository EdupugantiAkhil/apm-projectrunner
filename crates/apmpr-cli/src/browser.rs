...
const API_VERSION: &str = "apmpr.dev/managed-profile/v1alpha1";
const AUTH_OWNER_MARKER: &[u8] = b"apmpr-managed-profile-auth-v1\n";
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedBrowser => write!(
                formatter,
                "no supported Chromium build found; install Chromium or set APMPR_CHROMIUM to Chromium or Chrome for Testing (branded Chrome and Edge do not support the required extension launch flag)"
            ),
...
pub struct ManagedProfile {
    ...
    /// `apmpr.dev/managed-profile/v1alpha1`; the name is historical and carries no
    /// role meaning.
    #[serde(rename = "ui")]
    pub instance: String,
    pub route: String,
...
pub fn load_managed_profile(
    ...
    let generated_base = fs::canonicalize(workspace_root.join(".apmpr/generated"))?;
    require_contained(
        &canonical_workspace,
        &generated_base,
...
fn validate_start_url(value: &str) -> Result<(), BrowserError> {
    if apmpr_planner::is_local_http_url(value) {
        Ok(())
    } else {
...
fn ensure_profile_directory(
    ...
    let canonical_workspace = fs::canonicalize(workspace_root)?;
    let apmpr = workspace_root.join(".apmpr");
    let metadata = fs::symlink_metadata(&apmpr)?;
    if metadata.file_type().is_symlink() || !metadata.is_dir() {
        return Err(BrowserError::InvalidCredential(
            ".apmpr must be a real directory".into(),
        ));
    }
    let mut current = fs::canonicalize(apmpr)?;
    require_contained(
        &canonical_workspace,
        &current,
...
fn load_proxy_credential(
    ...
    let canonical_workspace = fs::canonicalize(workspace_root)?;
    let runtime_root = fs::canonicalize(workspace_root.join(".apmpr/run"))?;
    require_contained(
        &canonical_workspace,
        &runtime_root,
    ...
    require_contained(
        &runtime_root,
        &deployment_root,
        "deployment runtime directory escapes APM ProjectRunner runtime state",
    )?;
...
fn materialize_auth_extension(
    ...
    let requested = profile_dir.join(".apmpr-proxy-auth");
    ...
    let marker = directory.join(".apmpr-owned");
    if created {
    ...
    atomic_replace_owned(
        &directory,
        "manifest.json",
        br#"{"manifest_version":3,"name":"APM ProjectRunner managed proxy authentication","version":"0.1.0","permissions":["webRequest","webRequestAuthProvider"],"host_permissions":["<all_urls>"],"background":{"service_worker":"service-worker.js"}}"#,
    )?;
    let encoded = serde_json::to_string(credential)
        .map_err(|error| BrowserError::InvalidCredential(error.to_string()))?;
    let worker = format!(
        "const PASSWORD={encoded};chrome.webRequest.onAuthRequired.addListener((details,callback)=>{{if(details.isProxy){{callback({{authCredentials:{{username:'apmpr',password:PASSWORD}}}});}}else{{callback();}}}},{{urls:['<all_urls>']}},['asyncBlocking']);\n"
    );
...
fn find_browser() -> Result<PathBuf, BrowserError> {
    if let Some(configured) = env::var_os("APMPR_CHROMIUM") {
...
mod tests {
    ...
    fn existing_v1_metadata_spelling_the_instance_ui_still_parses() {
        ...
        let parsed: ManagedProfile = serde_json::from_str(
            r#"{"apiVersion":"apmpr.dev/managed-profile/v1alpha1","deployment":"comparison","ui":"backend-1","route":"backend-1","proxyAddress":"127.0.0.1:24001","startUrl":"http://backend-1.localhost/"}"#,
        )
    ...
    fn rejects_a_symlinked_generated_ancestor_that_escapes_workspace() {
        ...
        fs::create_dir_all(workspace.join(".apmpr")).unwrap();
        fs::create_dir_all(outside.join("comparison/managed-profiles")).unwrap();
        fs::write(
            outside.join("comparison/managed-profiles/ui-1.json"),
            r#"{"apiVersion":"apmpr.dev/managed-profile/v1alpha1","deployment":"comparison","ui":"ui-1","route":"ui-1","proxyAddress":"127.0.0.1:24001","startUrl":"http://ui-1.localhost/"}"#,
        )
        .unwrap();
        symlink(&outside, workspace.join(".apmpr/generated")).unwrap();

        let result = load_managed_profile(
            &workspace,
            Path::new(".apmpr/generated/comparison"),
            "comparison",
            "ui-1",
        );
    ...
    fn rejects_an_oversized_proxy_credential_before_reading_it() {
        ...
        let credential_dir = root.join(".apmpr/run/comparison/managed-profiles");
