...
use apmpr_docker_ssh::DockerSshTransport;
use serde::Deserialize;
use serde_json::Value;

pub const MANAGED_LABEL: &str = "dev.apmpr.managed";
pub const DEPLOYMENT_LABEL: &str = "dev.apmpr.deployment";
pub const RESOURCE_HASH_LABEL: &str = "dev.apmpr.resource-hash";
...
fn prepared_command(program: &str, arguments: &[String]) -> Command {
    ...
    if program == "docker" {
        if std::env::var_os("APMPR_ROUTER_TOKEN").is_none() {
            ...
            command.env("APMPR_ROUTER_TOKEN", "unused-for-compose-parsing");
        }
        if std::env::var_os("APMPR_UID").is_none() {
            if let Some(uid) = host_id("-u") {
                command.env("APMPR_UID", uid);
            }
        }
        if std::env::var_os("APMPR_GID").is_none() {
            if let Some(gid) = host_id("-g") {
                command.env("APMPR_GID", gid);
            }
        }
    }
    command
...
pub struct RuntimePlan {
    ...
    pub runtime_secrets: Vec<apmpr_planner::RuntimeSecretPlan>,
    ...
    pub external_probes: Vec<apmpr_planner::ExternalProbePlan>,
}
...
impl<E: CommandExecutor> DockerRuntime<E> {
    ...
    pub fn up(&self, plan: &RuntimePlan) -> Result<(), RuntimeError> {
        if plan.requires_router_token && std::env::var_os("APMPR_ROUTER_TOKEN").is_none() {
            return Err(RuntimeError::Docker {
                command: "docker compose up".into(),
                detail: "APMPR_ROUTER_TOKEN must be set".into(),
            });
        }
    ...
    fn probe_externals(&self, plan: &RuntimePlan) -> Result<(), RuntimeError> {
        ...
        for external in &plan.external_probes {
            let result = match &external.probe {
                apmpr_planner::Probe::Tcp { port } => probe_external_tcp(&external.host, *port),
                apmpr_planner::Probe::Http { path, port, https } => {
                ...
                apmpr_planner::Probe::Command { command } => {
...
pub fn verify_ownership(deployment: &str, resources: &[OwnedResource]) -> Result<(), RuntimeError> {
    for resource in resources {
        ...
        {
            return Err(RuntimeError::UnsafeCleanup(format!(
                "{} `{}` does not carry matching APM ProjectRunner ownership labels",
...
mod tests {
    ...
    fn remote_up_uses_ssh_environment_and_precedes_local_up() {
        ...
        assert_eq!(
            remote_streams[0].environment["APMPR_DOCKER_SSH_IDENTITY"],
    ...
    fn external_tcp_probe_has_a_distinct_reachability_error() {
        ...
        reachable
            .external_probes
            .push(apmpr_planner::ExternalProbePlan {
                instance: "host-db".into(),
                host: "127.0.0.1".into(),
                probe: apmpr_planner::Probe::Tcp { port },
            });
        ...
        drop(listener);
        reachable.external_probes[0].probe = apmpr_planner::Probe::Tcp { port: 0 };
        ...
        impl CommandExecutor for OrphanExecutor {
            fn capture(
                ...
                let stdout = if is_container_list && matches_project {
                    ...
                    // A container in the Compose project without APM ProjectRunner labels.
    ...
    fn parses_container_health_and_labels() {
        let response = r#"[{"Id":"abc","Name":"/demo-api","Config":{"Labels":{"dev.apmpr.managed":"true"}},"State":{"Status":"running","Health":{"Status":"healthy"}}}]"#;
