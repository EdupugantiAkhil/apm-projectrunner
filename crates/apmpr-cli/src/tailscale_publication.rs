...
use apmpr_adapters::TailscalePublicationAdapter;
use apmpr_planner::Plan;
use router_config::{GatewayExposureMode, RouterConfig};
use serde::{Deserialize, Serialize};
use serde_json::json;

const STATE_API_VERSION: &str = "apmpr.dev/tailscale-publication/v1alpha1";
...
impl<'a> TailscaleRuntime<'a> {
            ...
            Ok(record) => {
                ...
                match state {
                    Ok(Some(state)) if state.api_version == STATE_API_VERSION
                        ...
                        && state.record == record => {}
                    Ok(Some(_)) => checks.push(stale_check("persisted reachability differs from current tailnet or deployment state; run `apmpr up` to refresh it")),
                    Ok(None) => checks.push(stale_check("no persisted publication record exists; run `apmpr up` after the gateway is ready")),
    ...
    fn state_path(&self, create: bool) -> Result<PathBuf, TailscaleError> {
        ...
        for component in [".apmpr", "run", self.plan.deployment.as_str()] {
        ...
        let run_dir = root.join(".apmpr/run").join(&self.plan.deployment);
        if create {
