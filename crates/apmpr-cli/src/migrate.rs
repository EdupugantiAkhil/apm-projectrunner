...
use apmpr_planner::{API_VERSION, KIND};
use serde_yaml::Value;

const PREVIOUS_API_VERSION: &str = "apmpr.dev/v1alpha1";
/// Pre-rename spellings. `switchyard.dev` was the API group before the product was renamed to
/// APM ProjectRunner; the schema is unchanged, so migrating one is only an apiVersion rewrite.
const RENAMED_API_VERSION: &str = "switchyard.dev/v1alpha2";
const RENAMED_PREVIOUS_API_VERSION: &str = "switchyard.dev/v1alpha1";
...
pub fn migrate(
    ...
    let bundle = apmpr_planner::load_bundle_from_str(&migrated, path).map_err(|error| {
        MigrationError(format!(
    ...
    if let Err(diagnostics) = apmpr_planner::plan(&bundle) {
        return Err(MigrationError(format!(
...
mod tests {
    ...
    fn current_membership_fixture() -> Value {
        serde_yaml::from_str(include_str!(
            "../../apmpr-planner/tests/fixtures/deployment.yaml"
        ))
        .unwrap()
    }
    ...
    #[test]
    fn migrates_the_pre_rename_api_group_and_is_idempotent() {
        let directory = tempfile::tempdir().unwrap();
        for version in [RENAMED_API_VERSION, RENAMED_PREVIOUS_API_VERSION] {
            let path = directory.path().join("deployment.yaml");
            fs::write(
                &path,
                format!(
...
"#
            ...
            let result = migrate(&path, || {}).unwrap();
            assert!(result.changed, "{version} must migrate");
            assert_eq!(
                result.changes,
                [format!("apiVersion: {version} -> {API_VERSION}")]
            );
            // The remedy the loader's error names must actually produce a loadable document.
            let migrated = fs::read_to_string(&path).unwrap();
            let bundle = apmpr_planner::load_bundle(&path).unwrap();
            assert_eq!(bundle.api_version, API_VERSION);

            let second =
                migrate(&path, || panic!("an up-to-date file must not be rewritten")).unwrap();
    ...
    fn migrates_group_providers_and_is_idempotent() {
        ...
        fs::write(
            &path,
            r#"apiVersion: apmpr.dev/v1alpha1
kind: Deployment
metadata: { name: demo }
spec:
  repositories:
...
"#,
        ...
        assert_eq!(
            result
                ...
                .collect::<Vec<_>>(),
            [
                "apiVersion: apmpr.dev/v1alpha1 -> apmpr.dev/v1alpha2",
...
"#,
        ...
        let bundle = apmpr_planner::load_bundle(&path).unwrap();
        let plan = apmpr_planner::plan(&bundle).unwrap();
    ...
    fn refuses_multiple_ui_routes_for_one_group_without_writing() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("deployment.yaml");
        let original = r#"apiVersion: apmpr.dev/v1alpha2
kind: Deployment
metadata: { name: demo }
spec:
  uiRoutes:
...
"#;
    ...
    fn warning_hook_runs_before_destructive_rewrite() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("deployment.yaml");
        let original = r#"apiVersion: apmpr.dev/v1alpha1
kind: Deployment
metadata: { name: demo }
spec:
  repositories:
