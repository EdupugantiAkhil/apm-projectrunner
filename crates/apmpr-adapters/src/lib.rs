...
//!
//! Lifecycle adapters validate and plan resources while execution remains owned by APM ProjectRunner's
...
pub fn built_in_registry() -> AdapterRegistry {
    ...
    registry
        .register_route(ApmprRouteAdapter)
        .expect("built-in route-apmpr declaration is valid");
    registry
...
fn recover_handle(
    ...
    let Some(deployment) = labels.get("dev.apmpr.deployment") else {
        return Err(vec![Diagnostic::new(
            "adapter_recovery_label_missing",
            "$.dev.apmpr.deployment",
            "recovery requires the APM ProjectRunner deployment ownership label",
        )]);
    };
...
/// APM ProjectRunner route placement.
#[derive(Clone, Copy, Debug, Deserialize, JsonSchema, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum ApmprRouteMode {
...
/// APM ProjectRunner HTTP-family and TCP route configuration.
#[derive(Clone, Debug, Deserialize, JsonSchema, Serialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct ApmprRouteConfig {
    /// Placement used to realize the route.
    pub mode: ApmprRouteMode,
}

/// Sidecar and host-gateway route adapter for loopback HTTP-family and raw TCP slots.
#[derive(Clone, Copy, Debug)]
pub struct ApmprRouteAdapter;

impl Adapter for ApmprRouteAdapter {
    fn declaration(&self) -> AdapterDeclaration {
        declaration(
            "route-apmpr",
            vec![
    ...
    fn validate_configuration(&self, configuration: &Value) -> Vec<Diagnostic> {
        validate_typed::<ApmprRouteConfig>(configuration)
    }
...
impl RouteAdapter for ApmprRouteAdapter {
    fn validate(&self, context: &RouteValidationContext) -> Vec<Diagnostic> {
        ...
        if !is_loopback(&context.slot.host) {
            errors.push(Diagnostic::new(
                ...
                "$.slot.host",
                "APM ProjectRunner route slots must use localhost or a loopback IP address",
            ));
        }
        errors
    }
...
mod tests {
    ...
    use apmpr_adapter_sdk::{AdapterKind, RegisteredAdapter, conformance};
    ...
    conformance_test!(route_apmpr_conformance, AdapterKind::Route, "route-apmpr");
