...
use apmpr_planner::{load_bundle, load_bundle_from_str, plan};
use apmpr_router::{
        ...
        let task = tokio::spawn(async move {
            loop {
                ...
                tokio::spawn(async move {
                    ...
                    let body = if request.contains("x-apmpr-route:")
...
async fn group_domain_uses_its_default_and_allows_an_explicit_member_override() {
    ...
    let targeted = http_request(
        address,
        "GET / HTTP/1.1\r\nHost: feature.demo.localhost\r\nX-Apmpr-Route: alternate-member\r\nConnection: close\r\n\r\n",
    )
    .await;
    assert_eq!(response_body(&targeted), "alternate-member");
    assert!(!targeted.to_ascii_lowercase().contains("x-apmpr-route"));
    let unknown = http_request(
        address,
        "GET / HTTP/1.1\r\nHost: feature.demo.localhost\r\nX-Apmpr-Route: missing\r\nConnection: close\r\n\r\n",
    )
    .await;
...
async fn host_process_preserves_streaming_websocket_grpc_and_raw_tcp() {
    ...
    let request = Request::builder()
        .method("POST")
        .uri("http://localhost/apmpr.Greeter/Stream")
