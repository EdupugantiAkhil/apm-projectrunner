...
fn host_auth_precedes_config_access_but_certificate_commands_remain_tokenless() {
    ...
    for action in ["trust", "cleanup"] {
        ...
        assert!(
            !maintenance_error.contains("APMPR_ROUTER_TOKEN"),
            "{maintenance_error}"
        );
    }
}
