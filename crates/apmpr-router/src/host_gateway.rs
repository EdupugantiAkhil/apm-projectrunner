...
const CERTIFICATE_API_VERSION: &str = "apmpr.dev/certificate/v1alpha1";
...
/// Removes only proxy credentials carrying an APM ProjectRunner ownership marker.
...
/// Removes only certificate/key pairs carrying a valid APM ProjectRunner ownership marker.
...
pub fn trust_guidance(config: &RouterConfig) -> String {
    ...
    format!(
        "Generated certificates are self-signed and are not installed automatically.\n\
         ...
         Remove the copied trust-store file and run `sudo update-ca-certificates --fresh` before `apmpr-router certificates cleanup`.\n\
