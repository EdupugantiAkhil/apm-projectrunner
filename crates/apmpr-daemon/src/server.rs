...
pub trait OperationBackend: Send + Sync + 'static {
    ...
    fn run_action(
        &self,
        spec: apmpr_run_actions::OperationSpec,
        cancellation: watch::Receiver<bool>,
        events: EventSink,
    ) -> BackendFuture {
        match &spec {
            apmpr_run_actions::OperationSpec::Structured { command, .. } => {
                let kind = match command {
                    apmpr_run_actions::StructuredCommand::Up => CommandKind::Apply,
                    apmpr_run_actions::StructuredCommand::Down => CommandKind::Down,
                    apmpr_run_actions::StructuredCommand::Plan => CommandKind::Plan,
                    apmpr_run_actions::StructuredCommand::Status => CommandKind::Status,
                };
                let arguments = spec
                    .arguments()
            ...
            apmpr_run_actions::OperationSpec::Shell(_) => Box::pin(async {
                Err(ApiErrorV1::new(
                    "run_action_backend_unsupported",
                    "operation backend does not support shell run actions",
                ))
            }),
            apmpr_run_actions::OperationSpec::Membership { .. } => Box::pin(async {
                Err(ApiErrorV1::new(
                    "run_action_invalid",
...
/// Backend that executes the existing `apmpr` command implementation.
#[derive(Clone)]
    ...
    fn run(
        ...
        Box::pin(async move {
            let mut child = Command::new(&program)
                ...
                .env("APMPR_BYPASS_DAEMON", "1")
                .env("APMPR_ROUTER_TOKEN", router_token)
                .stdin(Stdio::null())
                .stdout(Stdio::piped())
                .stderr(Stdio::piped())
                .kill_on_drop(true)
                ...
                .map_err(|error| {
                    ApiErrorV1::new(
                        ...
                        format!("could not start apmpr command: {error}"),
                    )
                })?;
...
struct RouterTarget {
    ...
    endpoint: apmpr_router_admin::AdminEndpoint,
...
trait RouterAdmin {
    fn current_snapshot(
        ...
        endpoint: &apmpr_router_admin::AdminEndpoint,
        token: &str,
    ) -> Result<apmpr_router_admin::SnapshotIdentity, apmpr_router_admin::AdminError>;
    ...
    ) -> Result<apmpr_router_admin::ApplyAcknowledgement, apmpr_router_admin::AdminError>;
}
...
impl RouterAdmin for LocalRouterAdmin {
    fn current_snapshot(
        ...
        apmpr_router_admin::current_snapshot(endpoint, token)
    }
    ...
    fn apply_snapshot(
        ...
        apmpr_router_admin::apply_snapshot(endpoint, token, config)
    }
}
...
fn apply_live_binding_with_admin(
    ...
    let authored = apmpr_planner::load_bundle(&bundle_path)
    ...
    let authored_plan = apmpr_planner::plan_with_devices(&authored, &devices)
    ...
    let base = if resolved_path.is_file() {
        ...
        apmpr_planner::load_bundle(&resolved_path)
            .map_err(|error| ApiErrorV1::new("resolved_state_invalid", error.to_string()))?
    } else {
        authored
    };
    let mut plan =
        apmpr_planner::plan_with_membership_and_devices(&base, consumer, group, &devices)
            .map_err(|errors| ApiErrorV1::new("plan_failed", format_diagnostics(errors)))?;
    ...
    for (instance, encoded) in candidates {
        ...
        targets.push(RouterTarget {
            ...
            endpoint: apmpr_router_admin::AdminEndpoint::docker_exec(
                &sidecar.service,
    ...
    let host_dir = project_root.join(".apmpr/run").join(&plan.deployment);
    ...
    if let Some(encoded) = &plan.host_router_config {
        ...
        if comparable == old {
            ...
            targets.push(RouterTarget {
                ...
                route_instance: None,
                endpoint: apmpr_router_admin::AdminEndpoint::unix(host_socket),
                old,
                candidate,
            });
        }
    ...
    let mut applied: Vec<(usize, apmpr_router_admin::SnapshotIdentity)> = Vec::new();
        ...
        match admin.apply_snapshot(&target.endpoint, token, &target.candidate) {
            Ok(ack)
                if ack.version == version
                    ...
                    && ack.status == apmpr_router_admin::ActivationStatus::Activated =>
            {
                attempts.push(router_attempt(
                    &plan.deployment,
                    target,
    ...
    apmpr_planner::write_plan(project_root, &plan)
...
fn rollback_applied_targets(
    ...
    applied: Vec<(usize, apmpr_router_admin::SnapshotIdentity)>,
...
fn format_diagnostics(diagnostics: Vec<apmpr_planner::Diagnostic>) -> String {
    diagnostics
        .into_iter()
    ...
    R: tokio::io::AsyncRead + Unpin,
{
        ...
        .map_err(|error| ApiErrorV1::new("output_read_failed", error.to_string()))?
    {
        ...
        events.emit(
            event_kind,
            json!({"line": apmpr_planner::redact_event_line(&line), "stderr": stderr}),
        );
    }
    Ok(captured)
}
...
pub fn reconcile_project(project_root: &Path) -> Result<ReconciliationReport, DaemonError> {
    let state_dir = project_root.join(".apmpr");
    ...
    let mut resources = observe_docker()?
        ...
        .filter(|resource| {
            resource
                .labels
                .get(apmpr_state::DEPLOYMENT_LABEL)
        ...
        for (name, remote) in &manifest.remote_projects {
            match observe_docker_on(name, Some(&remote.device), true) {
                Ok(observed) => resources.extend(observed.into_iter().filter(|resource| {
                    resource.labels.get(apmpr_state::DEPLOYMENT_LABEL) == Some(&manifest.deployment)
                })),
                Err(_) => {
...
pub async fn start_with_backend(
    ...
    let discovery_path = config.project_root.join(".apmpr/daemon.json");
...
fn prepare(
    config: DaemonConfig,
    backend: Arc<dyn OperationBackend>,
    mut resources: Vec<apmpr_state::OwnedResourceObservation>,
    ...
    let state_dir = config.project_root.join(".apmpr");
    fs::create_dir_all(&state_dir)?;
    let (mut store, _) = StateStore::open(state_dir.join("state.sqlite3"))?;
    let now = now_millis();
...
/// real Docker Engine with APM ProjectRunner-labeled resources is present on the host.
#[doc(hidden)]
...
async fn list_adapters() -> Response {
    Json(apmpr_adapters::built_in_registry().list()).into_response()
}

fn run_action_contract(script: &apmpr_run_actions::RunScript) -> RunActionV1 {
...
fn run_actions_contract(root: &Path, actions: &[apmpr_run_actions::RunScript]) -> RunActionsV1 {
    RunActionsV1 {
        api_version: API_VERSION.into(),
        actions: actions.iter().map(run_action_contract).collect(),
        shell_notice_acknowledged: apmpr_run_actions::shell_notice_acknowledged(root),
    }
}

fn run_action_error_response(error: apmpr_run_actions::RunActionError) -> Response {
    let status = match error {
        apmpr_run_actions::RunActionError::NotFound { .. } => StatusCode::NOT_FOUND,
        apmpr_run_actions::RunActionError::DuplicateName { .. } => StatusCode::CONFLICT,
        apmpr_run_actions::RunActionError::InvalidFile { .. }
        | apmpr_run_actions::RunActionError::InvalidAction { .. } => {
            StatusCode::UNPROCESSABLE_ENTITY
        }
        apmpr_run_actions::RunActionError::Io { .. } => StatusCode::INTERNAL_SERVER_ERROR,
    };
...
fn structured_run_action_request(
...
) -> Result<apmpr_run_actions::RunScript, Box<Response>> {
    ...
    let script = apmpr_run_actions::RunScript {
        name,
        description,
...
async fn list_run_actions(State(inner): State<Arc<Inner>>) -> Response {
    blocking_source_response(move || {
        match apmpr_run_actions::load_result(&inner.config.project_root) {
            Ok(actions) => {
...
async fn create_run_action(
    ...
    blocking_source_response(move || {
        match apmpr_run_actions::create(&inner.config.project_root, script) {
            Ok(actions) => (
                StatusCode::CREATED,
...
async fn update_run_action(
    ...
    blocking_source_response(move || {
        let actions = match apmpr_run_actions::load_result(&inner.config.project_root) {
            Ok(actions) => actions,
            Err(error) => return run_action_error_response(error),
        };
        let Some(existing) = actions.iter().find(|action| action.name == name) else {
            return run_action_error_response(apmpr_run_actions::RunActionError::NotFound { name });
        };
        ...
        match apmpr_run_actions::update(&inner.config.project_root, &name, script) {
            Ok(actions) => {
                Json(run_actions_contract(&inner.config.project_root, &actions)).into_response()
            }
            Err(error) => run_action_error_response(error),
...
async fn delete_run_action(
    ...
    blocking_source_response(move || {
        ...
        match apmpr_run_actions::delete(&inner.config.project_root, &name) {
...
fn run_action_preview(
    inner: &Inner,
    script: &apmpr_run_actions::RunScript,
    request: &ExecuteRunActionRequestV1,
) -> Result<(RunActionPreviewV1, apmpr_run_actions::OperationSpec, String), Box<Response>> {
    ...
    let (target, deployment) = if script.command.is_some() {
        let loaded = apmpr_planner::load_bundle(&resolved_bundle).map_err(|error| {
            Box::new(api_error(
                StatusCode::UNPROCESSABLE_ENTITY,
    ...
    let spec = apmpr_run_actions::OperationSpec::from_script(script, bundle).map_err(|error| {
        Box::new(api_error(
            StatusCode::UNPROCESSABLE_ENTITY,
            "run_action_invalid",
            &error,
    ...
    let execution = match &spec {
        apmpr_run_actions::OperationSpec::Structured { .. } => RunActionExecutionV1::Structured {
            argv: spec
                .arguments()
                .expect("structured run action has arguments")
                .into_iter()
        ...
        apmpr_run_actions::OperationSpec::Shell(command) => RunActionExecutionV1::Shell {
            command: command.clone(),
        },
        apmpr_run_actions::OperationSpec::Membership { .. } => {
            unreachable!("run scripts cannot bind")
        }
    };
    let acknowledged = apmpr_run_actions::shell_notice_acknowledged(&inner.config.project_root);
...
async fn execute_run_action(
    ...
    let loaded = tokio::task::spawn_blocking(move || apmpr_run_actions::load_result(&root)).await;
    ...
    if script.is_shell() && !preview.shell_notice_acknowledged {
        ...
        let root = inner.config.project_root.clone();
        let acknowledged =
            tokio::task::spawn_blocking(move || apmpr_run_actions::acknowledge_shell_notice(&root))
                .await;
        match acknowledged {
            Ok(Ok(())) => {}
            Ok(Err(error)) => {
...
fn profile_records(
    root: &Path,
) -> Result<(Vec<ProfileRecord>, Vec<ProfileSourceErrorV1>), apmpr_profiles::ProfileError> {
    let mut records = Vec::new();
    let mut source_errors = Vec::new();
    for definition in profile_definition_paths(root) {
        let Ok(bundle) = apmpr_planner::load_bundle(&definition) else {
            continue;
        };
        let deployment = bundle.metadata.name;
        let listing = apmpr_profiles::list_profiles(root, &definition)?;
...
fn profile_origin(origin: &apmpr_profiles::ProfileOrigin) -> ProfileOriginV1 {
    match origin {
        apmpr_profiles::ProfileOrigin::Project => ProfileOriginV1::Project,
        apmpr_profiles::ProfileOrigin::ImportedFromSource { source, commit } => {
            ProfileOriginV1::ImportedFromSource {
                source: source.clone(),
                commit: commit.clone(),
            }
        }
        apmpr_profiles::ProfileOrigin::DiscoveredInSource { source, commit } => {
...
fn profile_trust(trust: apmpr_profiles::ProfileTrust) -> ProfileTrustV1 {
    match trust {
        apmpr_profiles::ProfileTrust::Trusted => ProfileTrustV1::Trusted,
        apmpr_profiles::ProfileTrust::Imported => ProfileTrustV1::Imported,
        apmpr_profiles::ProfileTrust::Changed => ProfileTrustV1::Changed,
        apmpr_profiles::ProfileTrust::NotImported => ProfileTrustV1::NotImported,
    }
}

fn profile_adapter(kind: apmpr_profiles::ProfileAdapterKind) -> ProfileAdapterKindV1 {
    match kind {
        apmpr_profiles::ProfileAdapterKind::Container => ProfileAdapterKindV1::Container,
        apmpr_profiles::ProfileAdapterKind::Script => ProfileAdapterKindV1::Script,
        apmpr_profiles::ProfileAdapterKind::ProcessCompose => ProfileAdapterKindV1::ProcessCompose,
    }
}
...
fn profile_origin_matches(
    origin: &apmpr_profiles::ProfileOrigin,
    selector: &ProfileSelectorV1,
) -> bool {
    match (origin, selector.origin) {
        (apmpr_profiles::ProfileOrigin::Project, ProfileOriginKindV1::Project) => true,
        (
            apmpr_profiles::ProfileOrigin::ImportedFromSource { source, .. },
            ProfileOriginKindV1::ImportedFromSource,
        )
        | (
            apmpr_profiles::ProfileOrigin::DiscoveredInSource { source, .. },
...
fn profile_error_response(error: apmpr_profiles::ProfileError) -> Response {
    let status = match error {
        apmpr_profiles::ProfileError::SourceNotFound { .. }
        | apmpr_profiles::ProfileError::ProfileNotFound { .. } => StatusCode::NOT_FOUND,
        apmpr_profiles::ProfileError::ManifestReviewChanged { .. } => StatusCode::CONFLICT,
        apmpr_profiles::ProfileError::MalformedManifest { .. }
        | apmpr_profiles::ProfileError::UnsupportedVersion { .. }
        | apmpr_profiles::ProfileError::InvalidProfile { .. }
        | apmpr_profiles::ProfileError::Definition(_) => StatusCode::UNPROCESSABLE_ENTITY,
        apmpr_profiles::ProfileError::Io { .. }
        | apmpr_profiles::ProfileError::State(_)
        | apmpr_profiles::ProfileError::Clock => StatusCode::INTERNAL_SERVER_ERROR,
    };
    api_error(status, error.code(), &error.to_string())
}
...
async fn profile_definition(
    ...
    blocking_source_response(move || {
        ...
        match apmpr_profiles::load_profile_block(
...
async fn profile_manifest_review(
    ...
    blocking_source_response(move || {
        match apmpr_profiles::review_source_profile_manifest(
            &inner.config.project_root,
            &query.source,
        ) {
...
async fn import_profile(
    ...
    blocking_source_response(move || {
        ...
        let importable = records.iter().any(|record| {
            record.row.name == name
                && match (&record.row.origin, record.row.trust) {
                    (
                        apmpr_profiles::ProfileOrigin::DiscoveredInSource { source, .. },
                        apmpr_profiles::ProfileTrust::NotImported,
                    )
                    | (
                        apmpr_profiles::ProfileOrigin::ImportedFromSource { source, .. },
                        apmpr_profiles::ProfileTrust::Changed,
        ...
        match apmpr_profiles::import_reviewed_source_profile(
            &inner.config.project_root,
            &request.source,
            &name,
    ...
    blocking_source_response(move || {
        ...
        if !records.iter().any(|record| {
            record.row.name == name
                && matches!(
                    record.row.origin,
                    apmpr_profiles::ProfileOrigin::ImportedFromSource { .. }
                )
        }) {
            return api_error(
                StatusCode::NOT_FOUND,
        ...
        match apmpr_profiles::remove_imported_profile(&inner.config.project_root, &name) {
...
fn validate_profile_blocking(
    ...
    if request.target_deployment.is_some()
        && !matches!(
            record.row.trust,
            apmpr_profiles::ProfileTrust::Trusted | apmpr_profiles::ProfileTrust::Imported
        )
    {
        return api_error(
            StatusCode::CONFLICT,
    ...
    let block = match apmpr_profiles::load_profile_block(
        &inner.config.project_root,
        &target_definition,
        name,
        &record.row.origin,
    ...
    let bundle = match apmpr_planner::load_bundle_from_str(&draft, &target_definition) {
        Ok(bundle) => bundle,
        Err(error) => {
            return Json(ProfileValidationV1 {
                api_version: API_VERSION.into(),
    ...
    let expanded_services = expanded_block
        ...
        .collect();
    let diagnostics = apmpr_planner::plan_with_devices(
        &bundle,
...
fn read_mdns_publication(root: &Path, deployment: &str) -> Option<MdnsPublicationV1> {
    ...
    if value.get("apiVersion")?.as_str()? != "apmpr.dev/mdns-publication/v1alpha1"
...
fn read_tailscale_publication(root: &Path, deployment: &str) -> Option<TailscalePublicationV1> {
    ...
    let value: Value = serde_json::from_slice(&fs::read(path).ok()?).ok()?;
    if value.get("apiVersion")?.as_str()? != "apmpr.dev/tailscale-publication/v1alpha1"
        || value.get("deployment")?.as_str()? != deployment
    {
        return None;
    }
...
fn validate_definition(
    ...
    let loaded = apmpr_planner::load_bundle(&temporary);
    let result = match loaded {
        Err(error) => Err(validation_error(
            "deployment definition is invalid",
            json!([{"code":"invalid_yaml","path":"$","message":error.to_string()}]),
        )),
        Ok(bundle) => match apmpr_planner::plan_with_devices(
            &bundle,
...
async fn deployment_detail(
    ...
    let reconciliation = inner
        ...
        .find(|entry| entry.deployment == deployment)
        .cloned()
        .unwrap_or_else(|| apmpr_state::DeploymentReconciliation {
            deployment: deployment.clone(),
            diagnostics: Vec::new(),
        });
    ...
    blocking_source_response(move || {
        ...
        match manager.register_unmanaged(&store, &request.name, &request.path) {
            Ok(source) => {
                ...
                (
                    StatusCode::CREATED,
                    Json(apmpr_sources::RegisteredSourceInspection { source, inspection }),
                )
                    .into_response()
            }
            Err(error) => source_api_error(error),
...
async fn check_device(
    ...
    blocking_source_response(move || {
        ...
        let (status, detail) = apmpr_devices::check_device_eligibility_with(&executor, &device);
        let store = inner
            .store
            .lock()
            .unwrap_or_else(|error| error.into_inner());
...
impl apmpr_devices::DeviceCheckExecutor for DaemonDeviceCheckExecutor {
    fn run(
        &self,
        program: &OsStr,
        arguments: &[OsString],
        environment: &BTreeMap<OsString, OsString>,
    ) -> io::Result<apmpr_devices::CheckOutput> {
...
fn registered_device_v1(
    ...
    let eligibility_reason = if eligibility == DeviceEligibilityV1::Eligible {
        ...
        apmpr_devices::eligibility_label(&device)
    };
    DeviceV1 {
        name: device.name,
...
fn device_placements(root: &Path, store: &StateStore) -> BTreeMap<String, Vec<DevicePlacementV1>> {
    ...
    for path in device_definition_paths(root) {
        if let Ok(bundle) = apmpr_planner::load_bundle(&path) {
    ...
    if let Ok(deployments) = store.deployments() {
        for deployment in deployments {
            ...
            let resolved = root
                ...
                .join("resolved-deployment.yaml");
            if let Ok(bundle) = apmpr_planner::load_bundle(&resolved) {
                add_bundle_placements(&mut placements, &bundle);
            }
        }
    }
...
async fn project_detail(State(inner): State<Arc<Inner>>) -> Response {
    match apmpr_state::load_project_metadata(&inner.config.project_root) {
        Ok(metadata) => {
...
enum OperationInvocation {
    ...
    RunAction(apmpr_run_actions::OperationSpec),
}
...
impl OperationInvocation {
    fn mutating(&self, kind: CommandKind) -> bool {
        match self {
            ...
            Self::RunAction(apmpr_run_actions::OperationSpec::Structured { command, .. }) => {
                matches!(
                    command,
                    apmpr_run_actions::StructuredCommand::Up
                        | apmpr_run_actions::StructuredCommand::Down
                )
            }
            Self::RunAction(
                apmpr_run_actions::OperationSpec::Shell(_)
                | apmpr_run_actions::OperationSpec::Membership { .. },
            ) => false,
        }
    }
    ...
    fn heavy(&self, kind: CommandKind) -> bool {
        match self {
            ...
            Self::RunAction(apmpr_run_actions::OperationSpec::Structured { command, .. }) => {
                matches!(command, apmpr_run_actions::StructuredCommand::Up)
            }
            Self::RunAction(
                apmpr_run_actions::OperationSpec::Shell(_)
                | apmpr_run_actions::OperationSpec::Membership { .. },
    ...
    fn instance(&self, kind: CommandKind, project_root: &Path) -> Option<String> {
        let (bundle, candidate, require_managed_profile) = match self {
            ...
            Self::RunAction(apmpr_run_actions::OperationSpec::Membership {
                bundle,
                instance,
                ..
            }) => (bundle.as_path(), instance.clone(), false),
            Self::RunAction(
                apmpr_run_actions::OperationSpec::Structured { .. }
                | apmpr_run_actions::OperationSpec::Shell(_),
            ) => return None,
        };
        ...
        let bundle = apmpr_planner::load_bundle(&bundle).ok()?;
...
async fn execute_clone_operation(
    ...
    cancellation: watch::Receiver<bool>,
    events: Arc<EventLog>,
    lock: apmpr_state::OperationLock,
) {
    ...
    let state_path = project_root.join(".apmpr/state.sqlite3");
...
async fn execute_operation(
    ...
    cancellation: watch::Receiver<bool>,
    events: Arc<EventLog>,
    mut lock: Option<apmpr_state::OperationLock>,
) {
    ...
    let (mut work, persistence_bundle) = match invocation {
        ...
        OperationInvocation::RunAction(spec) => {
            let persistence_bundle = match &spec {
                apmpr_run_actions::OperationSpec::Structured {
                    command: apmpr_run_actions::StructuredCommand::Up,
                    bundle,
                    ..
                } => Some(bundle.clone()),
                _ => None,
    ...
    match outcome {
        ...
        Ok(BackendOutcome::Completed(result)) => finish_operation(
            ...
            OperationStatusV1::Failed,
            Some(result),
            lock_lost.or_else(|| Some(ApiErrorV1::new("command_failed", "apmpr command failed"))),
            &events,
            lock,
        ),
...
fn persist_applied_snapshot(inner: &Inner, bundle: &Path) -> Result<(), ApiErrorV1> {
    ...
    let bundle = apmpr_planner::load_bundle(&bundle_path)
        .map_err(|error| ApiErrorV1::new("bundle_load_failed", error.to_string()))?;
    let devices = planner_devices(&inner.config.project_root)
        .map_err(|error| ApiErrorV1::new("device_load_failed", error.to_string()))?;
    let authored_plan = apmpr_planner::plan_with_devices(&bundle, &devices)
        .map_err(|errors| ApiErrorV1::new("plan_failed", format_diagnostics(errors)))?;
    ...
    let resolved = apmpr_planner::load_bundle(&resolved_path)
        .map_err(|error| ApiErrorV1::new("resolved_state_invalid", error.to_string()))?;
    let applied_plan = apmpr_planner::plan_with_devices(&resolved, &devices)
        .map_err(|errors| ApiErrorV1::new("plan_failed", format_diagnostics(errors)))?;
...
fn finish_operation(
    ...
    events: &EventLog,
    lock: Option<apmpr_state::OperationLock>,
) {
    ...
    let state_error = error.as_ref().and_then(|error| {
        StructuredContext::new(error.context.clone().unwrap_or_else(|| json!({})))
            .ok()
            .map(|context| apmpr_state::OperationError {
...
fn deployment_id(bundle_path: &Path) -> String {
    apmpr_planner::load_bundle(bundle_path).ok().map_or_else(
        || {
...
fn planner_devices(
    project_root: &Path,
) -> Result<BTreeMap<String, apmpr_planner::PlanningDevice>, StateError> {
    let (store, _) = StateStore::open(project_root.join(".apmpr/state.sqlite3"))?;
    Ok(store
        .devices()?
        .into_iter()
        .map(|device| {
...
fn operation_from_stored(stored: apmpr_state::StoredOperation) -> Result<OperationV1, ApiErrorV1> {
...
fn load_or_create_router_token(project_root: &Path) -> Result<String, DaemonError> {
    let configured = std::env::var_os("APMPR_ROUTER_TOKEN")
        .map(|value| {
            value.into_string().map_err(|_| {
                DaemonError::InvalidConfiguration("APMPR_ROUTER_TOKEN must be valid UTF-8".into())
            })
        })
        .transpose()?;
...
fn load_or_create_router_token_with(
    ...
    match fs::symlink_metadata(&path) {
        Ok(metadata) => {
            ...
            {
                return Err(DaemonError::InvalidConfiguration(
                    "APMPR_ROUTER_TOKEN does not match the project router credential; stop existing routers and remove .apmpr/router-token before rotating it".into(),
                ));
            }
            Ok(token)
        }
...
fn observe_docker() -> Result<Vec<apmpr_state::OwnedResourceObservation>, io::Error> {
    observe_docker_on("local", None, false)
}

fn observe_docker_on(
    device_name: &str,
    device: Option<&apmpr_state::GeneratedDevice>,
    strict: bool,
) -> Result<Vec<apmpr_state::OwnedResourceObservation>, io::Error> {
    let mut observations = Vec::new();
    let docker_transport = device
        .map(|device| {
            apmpr_docker_ssh::DockerSshTransport::new(
                &device.user,
                &device.host,
                device.port,
    ...
    for (kind, noun, supports_all) in [
        (apmpr_state::ResourceKind::Container, "container", true),
        (apmpr_state::ResourceKind::Image, "image", true),
        (apmpr_state::ResourceKind::Network, "network", false),
        (apmpr_state::ResourceKind::Volume, "volume", false),
    ] {
        let mut arguments = vec![noun, "list"];
        if supports_all {
            arguments.push("--all");
        }
        arguments.extend(["--filter", "label=dev.apmpr.managed=true", "--quiet"]);
            ...
            .filter(|id| !id.is_empty())
        {
            ...
            let labels = labels_value
                ...
                .unwrap_or_default();
            observations.push(apmpr_state::OwnedResourceObservation {
                kind,
                id: value
                    .get("Id")
...
fn configure_docker_device(
    ...
    transport: Option<&apmpr_docker_ssh::DockerSshTransport>,
) {
...
mod tests {
    ...
    impl RouterAdmin for FailingRollbackAdmin {
        fn current_snapshot(
            ...
            if endpoint == &apmpr_router_admin::AdminEndpoint::unix("second.socket") {
                return Err(apmpr_router_admin::AdminError::InvalidResponse(
                    "observation unavailable".into(),
                ));
            }
            Ok(apmpr_router_admin::SnapshotIdentity {
                id: "active".into(),
                version: 2,
        ...
        ) -> Result<apmpr_router_admin::ApplyAcknowledgement, apmpr_router_admin::AdminError>
        {
            let rendered = match endpoint {
                apmpr_router_admin::AdminEndpoint::Unix(path) => {
                    path.to_string_lossy().into_owned()
                }
                apmpr_router_admin::AdminEndpoint::DockerExec { container, .. } => {
                    container.clone()
                }
            };
            self.applied
                ...
                .unwrap_or_else(|error| error.into_inner())
                .push(rendered);
            Ok(apmpr_router_admin::ApplyAcknowledgement {
                version: config.spec.snapshot.version,
                checksum: snapshot_checksum(config),
                status: apmpr_router_admin::ActivationStatus::Activated,
            })
        }
    }
    ...
    fn rollback_observation_failure_is_recorded_and_remaining_targets_continue() {
        ...
        let target = |id: &str, socket: &str| {
            ...
            RouterTarget {
                id: id.into(),
                route_instance: None,
                endpoint: apmpr_router_admin::AdminEndpoint::unix(socket),
                old: config.clone(),
                candidate,
            }
        };
        ...
        let observed = |id: &str| apmpr_router_admin::SnapshotIdentity {
            id: id.into(),
            version: 1,
    ...
    async fn cli_backend_supplies_the_managed_router_token() {
        ...
        let outcome = backend
            .run(
                ...
                vec![
                    "-c".into(),
                    "test \"$APMPR_ROUTER_TOKEN\" = managed-router-token && test \"$APMPR_BYPASS_DAEMON\" = 1".into(),
                ],
                cancellation,
                EventSink {
    ...
    fn router_credential_is_private_stable_and_rejects_mismatched_override() {
        ...
        let path = temp.path().join(".apmpr/router-token");
        assert_eq!(
