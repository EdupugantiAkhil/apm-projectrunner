...
impl OperationBackend for StubBackend {
    fn run(
        ...
        Box<dyn Future<Output = Result<BackendOutcome, apmpr_daemon::contract::ApiErrorV1>> + Send>,
    > {
        Box::pin(async move {
            for kind in [
                apmpr_daemon::contract::EventKindV1::Build,
                apmpr_daemon::contract::EventKindV1::Health,
                apmpr_daemon::contract::EventKindV1::Route,
                apmpr_daemon::contract::EventKindV1::Log,
            ] {
...
struct RunActionBackend {
    invocations: Arc<Mutex<Vec<apmpr_run_actions::OperationSpec>>>,
}
...
impl OperationBackend for LockLossBackend {
    ...
    fn live_bind(
        &self,
        request: apmpr_daemon::contract::CommandRequestV1,
        operation_id: String,
        mut cancellation: watch::Receiver<bool>,
        _events: EventSink,
    ) -> Option<
        Pin<
            Box<
                dyn Future<Output = Result<BackendOutcome, apmpr_daemon::contract::ApiErrorV1>>
                    + Send,
            >,
        >,
    > {
...
fn fixture() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("../apmpr-planner/tests/fixtures/deployment.yaml")
}
...
fn second_fixture(temp: &TempDir) -> PathBuf {
    ...
    fs::copy(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../apmpr-planner/tests/fixtures/process-compose.yaml"),
...
struct TestApi {
    router: Router,
    token: String,
    reconciliation: apmpr_state::ReconciliationReport,
}
...
async fn run_actions_list_structured_crud_reject_shell_authoring_and_gate_shell_execution() {
    ...
    fs::create_dir_all(temp.path().join(".apmpr")).unwrap();
    fs::write(
        temp.path().join(apmpr_run_actions::FILE_NAME),
    ...
    assert!(!apmpr_run_actions::shell_notice_acknowledged(temp.path()));
    ...
    assert!(matches!(
        ...
        [
            ...
            apmpr_run_actions::OperationSpec::Structured {
                command: apmpr_run_actions::StructuredCommand::Status,
                ..
            }
        ]
    ));
...
async fn profile_library_enforces_reviewed_import_reimport_validation_and_removal() {
    ...
    fs::write(
        repository.join("apmpr-profiles.yaml"),
        manifest("busybox:1"),
    )
    .unwrap();
    for arguments in [
        vec!["add", "apmpr-profiles.yaml"],
    ...
    let status = std::process::Command::new("git")
        ...
        .unwrap();
    assert!(status.success());
    let (store, _) = StateStore::open(temp.path().join(".apmpr/state.sqlite3")).unwrap();
    store
    ...
    fs::write(checkout.join("apmpr-profiles.yaml"), manifest("busybox:2")).unwrap();
    let (status, body) = request(
        &api,
        Some(&api.token),
        "POST",
    ...
    fs::write(checkout.join("apmpr-profiles.yaml"), manifest("busybox:3")).unwrap();
...
async fn durable_operation_list_filters_and_marks_destructive_records() {
    let temp = TempDir::new().unwrap();
    let state_path = temp.path().join(".apmpr/state.sqlite3");
...
async fn deployment_and_adapter_endpoints_are_authenticated_and_shape_empty_state() {
    ...
    fs::create_dir(temp.path().join(".apmpr")).unwrap();
    fs::write(
        temp.path().join(apmpr_state::PROJECT_MARKER_PATH),
        r#"{"apiVersion":"apmpr.dev/v1alpha1","name":"browser-project"}"#,
    )
    .unwrap();
    let api = start_api(&temp, Arc::new(ImmediateBackend), 1);
...
async fn high_concurrency_api_respects_global_limit_deployment_locks_and_sqlite_consistency() {
    ...
    let reopened = StateStore::open(temp.path().join(".apmpr/state.sqlite3"))
        .unwrap()
        .0;
...
async fn deployment_list_and_detail_include_applied_manifest_and_reconciliation() {
    let temp = tempfile::tempdir().unwrap();
    fs::create_dir_all(temp.path().join(".apmpr/generated/demo")).unwrap();
    let (mut store, _) = StateStore::open(temp.path().join(".apmpr/state.sqlite3")).unwrap();
    ...
    store
        ...
        .unwrap();
    fs::write(
        temp.path().join(".apmpr/generated/demo/manifest.json"),
    ...
    .unwrap();
    fs::create_dir_all(temp.path().join(".apmpr/run/demo")).unwrap();
    fs::write(
        temp.path().join(".apmpr/run/demo/mdns-publication.json"),
        serde_json::to_vec(&json!({
            "apiVersion": "apmpr.dev/mdns-publication/v1alpha1",
            "deployment": "demo",
            "definitionHash": "definition-1",
            "publishers": [{
                "pid": 42,
    ...
    fs::write(
        temp.path()
            .join(".apmpr/run/demo/tailscale-publication.json"),
        serde_json::to_vec(&json!({
            "apiVersion": "apmpr.dev/tailscale-publication/v1alpha1",
            "deployment": "demo",
            "definitionHash": "definition-1",
            "record": {
                "scope": "tailnet",
...
async fn auth_and_versioned_surface_are_enforced() {
    ...
    let error: apmpr_daemon::contract::ApiErrorV1 = json_body(&malformed.1);
...
async fn lock_loss_cancels_live_binding_then_persists_its_attempts() {
    ...
    let (mut competing_store, _) =
        StateStore::open(temp.path().join(".apmpr/state.sqlite3")).unwrap();
    ...
    .await;
    let routes: apmpr_daemon::contract::DeploymentRoutesV1 = json_body(&body);
...
async fn failed_live_binding_versions_and_rollback_history_survive_restart() {
    ...
    .await;
    let restored: apmpr_daemon::contract::DeploymentRoutesV1 = json_body(&body);
...
async fn applied_domains_bindings_and_deleted_database_recovery_survive_daemon_restart() {
    ...
    let authored = apmpr_planner::load_bundle(&bundle).unwrap();
    let plan = apmpr_planner::plan(&authored).unwrap();
    apmpr_planner::write_plan(temp.path(), &plan).unwrap();
    let daemon = start_api(&temp, Arc::new(StubBackend), 2);
    let (_, operation) = request(
        &daemon,
        Some(&daemon.token),
    ...
    let (store, _) =
        apmpr_state::StateStore::open(temp.path().join(".apmpr/state.sqlite3")).unwrap();
    ...
    fs::remove_file(temp.path().join(".apmpr/state.sqlite3")).unwrap();
    ...
    assert!(
        deployment
            .diagnostics
            .iter()
            .any(|diagnostic| diagnostic.code == apmpr_state::DriftCode::AppliedStateMissing)
    );
}
...
async fn source_and_worktree_endpoints_enforce_auth_validation_and_non_destructive_errors() {
    ...
    .await;
    assert_eq!(status, 201, "{}", String::from_utf8_lossy(&body));
    let created: apmpr_daemon::contract::SourceV1 = json_body(&body);
    ...
    let error: apmpr_daemon::contract::ApiErrorV1 = json_body(&body);
    ...
    let worktrees: Vec<apmpr_daemon::contract::WorktreeV1> = json_body(&body);
...
async fn clone_operation_registers_source_without_persisting_or_streaming_credentials() {
    ...
    let sqlite = fs::read(temp.path().join(".apmpr/state.sqlite3")).unwrap();
    assert!(
        !sqlite
            .windows(secret.len())
            .any(|window| window == secret.as_bytes())
    ...
    .await;
    let sources: Vec<apmpr_daemon::contract::SourceV1> = json_body(&sources);
