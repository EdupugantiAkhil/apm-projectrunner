use apmpr_ops::{RunScript, run_scripts::StructuredCommand};
use appcui::prelude::*;
...
impl ApmprShell {
    ...
    pub(crate) fn edit_script(&mut self, new: bool) {
        ...
        match apmpr_ops::run_scripts::save(&self.state.project_dir, &scripts) {
            Ok(()) => {
    ...
    pub(crate) fn remove_script(&mut self) {
        ...
        if !confirm::safe_remove(&format!(
            "Delete project run action `{name}`?\n\nThis edits .apmpr/run-scripts.yaml. It does not remove a startup profile or stop services."
        )) {
            return;
        }
        let mut scripts = self.state.run_scripts.clone();
    ...
    pub(crate) fn run_script(&mut self) {
        ...
        let Ok(spec) = apmpr_ops::execution::OperationSpec::from_script(&script, bundle) else {
        ...
        if matches!(spec, apmpr_ops::execution::OperationSpec::Shell(_))
            && !apmpr_ops::run_scripts::shell_notice_acknowledged(&self.state.project_dir)
        {
            let warning = "This project run action executes a shell command with your user permissions from the project directory. Review .apmpr/run-scripts.yaml before continuing. Acknowledgement is remembered for this project.";
            if !dialogs::validate("Shell command warning", warning) {
                return;
            }
            if let Err(error) =
                apmpr_ops::run_scripts::acknowledge_shell_notice(&self.state.project_dir)
            {
                self.set_notices(&format!(
...
impl TextFieldEvents for ApmprShell {
...
pub(crate) fn apply_operation_update(shell: &mut ApmprShell, update: OpUpdate) {
    match update {
        OpUpdate::Started {
            label,
            deployment,
...
pub(crate) fn finish_operation_task(shell: &mut ApmprShell) {
...
pub(crate) fn add(tab: &mut Tab, index: u32, state: &ProjectState) -> Handles {
    ...
    let mut top = Panel::new(
        "Project run actions — .apmpr/run-scripts.yaml",
        layout!("d:f"),
    );
...
impl ScriptDialog {
    pub(crate) fn new(script: Option<&RunScript>, reserved_names: Vec<String>) -> Self {
        ...
        mode.add("Structured APM ProjectRunner command");
...
mod tests {
    ...
    fn timeline_filter_matches_deployment_instance_and_service_output() {
        ...
        log.append("Warning [listener_collision] group dual-write port 5432: routing to db-main, the first listed");
        ...
        assert!(render_timeline(&log, "listener_collision").contains("routing to db-main"));
