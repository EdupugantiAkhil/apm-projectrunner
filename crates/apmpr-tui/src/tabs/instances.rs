...
use apmpr_ops::{execution::OperationSpec, run_scripts::StructuredCommand};
use appcui::prelude::*;
...
impl ApmprShell {
    ...
    pub(crate) fn cleanup_instance(&mut self) {
        ...
        let preview = format!(
            "CLEAN UP deployment `{}`?\n\nThis stops every service and PERMANENTLY DELETES APM ProjectRunner-owned named volumes for this deployment. Volume data cannot be recovered by APM ProjectRunner.\n\nDefinition files and unmanaged source directories are not deleted.",
            deployment.name
        );
...
fn cli_shell_spec(command: &str, bundle: &Path, confirmed: bool) -> OperationSpec {
    let executable = std::env::var_os("APMPR_BIN")
        .map(PathBuf::from)
