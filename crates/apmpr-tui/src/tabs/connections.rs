use apmpr_ops::{ConnectionRow, RouteStatus, SwitchPreview, execution::OperationSpec};
use appcui::prelude::*;

use crate::{
    shell::{ApmprShell, PendingMembershipMove},
    state::ProjectState,
};
...
impl ApmprShell {
    ...
    pub(crate) fn switch_connection(&mut self) {
        ...
        for group in groups {
            match apmpr_ops::switch_preview(&self.state.project_dir, &bundle, &instance, &group) {
    ...
    pub(crate) fn finish_membership_move(&mut self, succeeded: bool, operation_detail: &str) {
        ...
        let (statuses, status_error) =
            match apmpr_ops::route_status(&self.state.project_dir, &membership_move.deployment) {
...
mod tests {
    ...
    use apmpr_ops::{MembershipChange, RouteHistoryEntry};
    ...
    fn matrix_projection_surfaces_unbound_fix_key_and_route_failure() {
        ...
        let state = ProjectState {
            deployments: vec![DeploymentProjection {
                connections: apmpr_ops::ConnectionMatrix {
                    rows: vec![connection()],
                },
