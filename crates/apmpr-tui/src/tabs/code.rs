...
use apmpr_state::RegisteredSourceKind;
use appcui::prelude::*;

use crate::{
    dialogs::confirm,
    handoff::CloneHandoff,
    shell::{ApmprShell, StateAction},
...
impl ApmprShell {
    ...
    pub(crate) fn remove_code(&mut self) {
        ...
        let preview = match source.ownership {
            apmpr_state::RegisteredSourceKind::Managed => format!(
                "Remove and deregister `{}`?\n\nManaged path to delete: {}\nAPM ProjectRunner will refuse if ownership, containment, or clean-state checks fail.",
                source.name,
                source.path.display(),
            ),
            apmpr_state::RegisteredSourceKind::Unmanaged => format!(
...
pub(crate) fn add(
    ...
    let mut empty = Label::new(
        "Code is a repository, checkout, or worktree that APM ProjectRunner can use for an instance.\n\nPress F2 to register an existing directory or clone a repository.",
...
pub(crate) fn detail_text(state: &ProjectState, source: &SourceProjection) -> String {
    ...
    let ownership = match source.ownership {
        RegisteredSourceKind::Managed => "managed by APM ProjectRunner",
...
impl WorktreeDialog {
    pub(crate) fn new(
        ...
        dialog.add(Label::new(
            "Leave path empty for .apmpr/worktrees/<branch>.",
