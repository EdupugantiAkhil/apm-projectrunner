...
impl ApmprShell {
    ...
    pub(crate) fn remove_profile(&mut self) {
        ...
        if !matches!(
            profile.row.origin,
            apmpr_ops::ProfileOrigin::ImportedFromSource { .. }
        ) {
...
pub(crate) fn add(tab: &mut Tab, index: u32, state: &ProjectState) -> Handles {
    ...
    let mut empty = Label::new(
        "A startup profile is a reusable definition of one service or a coordinated suite.\n\nPress F2 to start a project profile, or register a source containing apmpr-profiles.yaml and press F5 to discover it.",
...
pub(crate) fn show_editor(profile: Option<&ProfileProjection>) {
    let registry = apmpr_adapters::built_in_registry();
    ...
    if let Some(values) = SchemaFormDialog::new(&title, schema, &initial).show() {
        ...
        ReportDialog::new("Guided editor preview", &format!("# Adapter configuration preview\n\n```\n{yaml}```\n\nSaving is unavailable because apmpr-ops does not yet expose a profile create/update mutation.")).show();
    }
}
...
mod tests {
    use super::*;
    use apmpr_ops::{ProfileRow, ProfileService};
