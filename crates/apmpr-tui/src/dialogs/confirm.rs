...
impl DestructiveConfirm {
    fn new(title: &str, action: &str, preview: &str) -> Self {
        let mut dialog = Self {
        ...
        dialog.remove = dialog.add(Button::new(action, layout!("x:35%,y:100%,p:b,w:16,h:1")));
...
pub(crate) fn safe_remove(preview: &str) -> bool {
    DestructiveConfirm::new("Confirm safe removal", "&Remove", preview)
        .show()
        .unwrap_or(false)
}

pub(crate) fn destructive_cleanup(preview: &str) -> bool {
    DestructiveConfirm::new("DESTRUCTIVE cleanup", "&Cleanup", preview)
        .show()
        .unwrap_or(false)
}
