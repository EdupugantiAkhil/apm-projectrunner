...
pub(crate) fn execute_interactive_clone(root: &Path, request: CloneHandoff) -> Result<(), String> {
    println!("APM ProjectRunner yielded the terminal to Git.\n");
...
fn clone_source(root: &Path, request: CloneHandoff) -> Result<(), String> {
    ...
    let store = StateStore::open(root.join(".apmpr/state.sqlite3"))
        .map_err(|error| error.to_string())?
        .0;
    let manager = apmpr_sources::SourceManager::new(root);
