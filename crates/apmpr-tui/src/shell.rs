...
use apmpr_ops::{create_instance, execution::OperationSpec};
use apmpr_state::{DeviceCheckStatus, RegisteredDevice, StateStore};
use code::TreeRow;
...
fn execute_state_job(connector: &BackgroundTaskConector<StateUpdate, TaskResponse>) {
    ...
    let notice = match job.action {
        ...
        StateAction::Remove { name } => Some(
            state
                .remove_source(&name)
                .map(|()| "Source removed from APM ProjectRunner.".into())
                .unwrap_or_else(|error| format!("Safe removal refused: {error}")),
        ),
        StateAction::ImportProfile { source, name } => Some(
            apmpr_ops::import_source_profile(&job.project_dir, &source, &name)
        ...
        StateAction::RemoveProfile { name } => Some(
            apmpr_ops::remove_imported_profile(&job.project_dir, &name)
        ...
        StateAction::CheckDevice(name) => Some(
            (|| {
                let (store, _) = StateStore::open(job.project_dir.join(".apmpr/state.sqlite3"))
        ...
        StateAction::RemoveDevice(name) => Some(
            StateStore::open(job.project_dir.join(".apmpr/state.sqlite3"))
                .map_err(|error| error.to_string())
                .and_then(|(store, _)| {
                    store
...
impl ApmprShell {
    fn new(project_dir: &Path, outcome: OutcomeCell) -> Self {
        ...
        let title = format!("APM ProjectRunner — {}", project_dir.display());
        let mut shell = Self {
    ...
    fn try_quit(&mut self) {
        if !self.operation_gate.is_running()
            || dialogs::validate(
                ...
                "An operation is still running. Quit APM ProjectRunner anyway?",
            )
        {
            self.close();
        }
...
impl ButtonEvents for ApmprShell {
...
impl TimerEvents for ApmprShell {
...
impl WindowEvents for ApmprShell {
...
impl TreeViewEvents<code::TreeRow> for ApmprShell {
...
impl ListViewEvents<ProfileRowView> for ApmprShell {
    fn on_current_item_changed(
        &mut self,
...
impl ListViewEvents<InstanceRowView> for ApmprShell {
    fn on_current_item_changed(
        &mut self,
...
impl ListViewEvents<ConnectionRowView> for ApmprShell {
    fn on_item_action(
        &mut self,
...
impl ListViewEvents<DeviceRowView> for ApmprShell {
    fn on_current_item_changed(
        &mut self,
...
impl ListViewEvents<ScriptRowView> for ApmprShell {
    fn on_item_action(
        &mut self,
...
impl BackgroundTaskEvents<StateUpdate, TaskResponse> for ApmprShell {
    fn on_update(
        &mut self,
        update: StateUpdate,
        ...
        if let Some(device) = update.device_probe {
            ...
            if dialogs::validate(
                ...
                let saved = StateStore::open(self.state.project_dir.join(".apmpr/state.sqlite3"))
                    .map_err(|error| error.to_string())
                    .and_then(|(store, _)| {
                        store
...
impl BackgroundTaskEvents<OpUpdate, OpCommand> for ApmprShell {
    fn on_update(
        &mut self,
        update: OpUpdate,
...
impl CommandBarEvents for ApmprShell {
    fn on_update_commandbar(&self, commandbar: &mut CommandBar) {
        use apmprshell::Commands as C;
    ...
    fn on_event(&mut self, command_id: apmprshell::Commands) {
        match command_id {
            apmprshell::Commands::Help => {
                HelpDialog::new().show();
            }
            apmprshell::Commands::Refresh => self.refresh_state(),
            apmprshell::Commands::Quit => self.try_quit(),
            apmprshell::Commands::Next => self.navigate_to_next_action(),
            apmprshell::Commands::AddCode => self.add_code(),
            apmprshell::Commands::Worktree => self.create_worktree(),
            apmprshell::Commands::RemoveCode => self.remove_code(),
            apmprshell::Commands::CodeDetails => self.show_code_details(),
            apmprshell::Commands::NewProfile => self.edit_profile(true),
            apmprshell::Commands::EditProfile => self.edit_profile(false),
            apmprshell::Commands::ValidateProfile => self.validate_profile(),
            apmprshell::Commands::ImportProfile => self.import_profile(),
            apmprshell::Commands::RemoveProfile => self.remove_profile(),
            apmprshell::Commands::ProfileDetails => self.show_profile_details(),
            apmprshell::Commands::NewInstance => self.create_instance(),
            apmprshell::Commands::InstanceDetails => self.show_instance_details(),
            apmprshell::Commands::ValidateInstance => {
                self.run_instance_command(InstanceCommand::Validate)
            }
            apmprshell::Commands::PlanInstance => self.run_instance_command(InstanceCommand::Plan),
            apmprshell::Commands::StartInstance => {
                self.run_instance_command(InstanceCommand::Start)
            }
            apmprshell::Commands::StopInstance => self.run_instance_command(InstanceCommand::Stop),
            apmprshell::Commands::CleanupInstance => self.cleanup_instance(),
            apmprshell::Commands::SwitchConnection => self.switch_connection(),
            apmprshell::Commands::AddDevice => self.add_device(),
            apmprshell::Commands::CheckDevice => self.check_device(),
            apmprshell::Commands::RemoveDevice => self.remove_device(),
            apmprshell::Commands::DeviceDetails => self.show_device_details(),
            apmprshell::Commands::NewScript => self.edit_script(true),
            apmprshell::Commands::EditScript => self.edit_script(false),
            apmprshell::Commands::RemoveScript => self.remove_script(),
            apmprshell::Commands::RunScript => self.run_script(),
        }
    }
}
...
pub(crate) fn run_app(
    ...
    app.add_window(ApmprShell::new(project_dir, outcome.clone()));
    app.run();
    Ok(outcome.take())
}
