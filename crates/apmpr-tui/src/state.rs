...
use apmpr_ops::profiles::{ProfileOrigin, ProfileRow};
use apmpr_ops::{
...
use apmpr_state::{RegisteredSourceKind, StateStore};
...
pub(crate) struct SourceProjection {
    ...
    pub(crate) inspection: apmpr_sources::SourceInspection,
...
impl Default for SourceProjection {
    fn default() -> Self {
        let path = PathBuf::from("/apmpr-test-source-does-not-exist");
        Self {
            name: String::new(),
            path: path.clone(),
            ...
            inspection: SourceManager::new("/apmpr-test-workspace").inspect(&path, None),
            available: false,
            inspected_at: 0,
        }
    }
...
pub(crate) struct DeviceProjection {
    pub(crate) name: String,
    pub(crate) device: apmpr_state::RegisteredDevice,
}
    ...
    fn default() -> Self {
        Self {
            ...
            row: ProfileRow {
                ...
                trust: apmpr_ops::ProfileTrust::Trusted,
                shadowed: false,
                services: Vec::new(),
            },
...
pub(crate) struct DeploymentProjection {
    ...
    pub(crate) source_choices: Vec<apmpr_ops::SourceChoice>,
    pub(crate) connections: ConnectionMatrix,
    pub(crate) route_statuses: Vec<apmpr_ops::RouteStatus>,
...
impl ProjectState {
    ...
    fn refresh_profiles(&mut self, deployments: &[apmpr_ops::DeploymentEntry]) {
        self.profiles.clear();
    ...
    fn refresh_connections(&mut self, deployments: &[apmpr_ops::DeploymentEntry]) {
