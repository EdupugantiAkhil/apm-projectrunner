...
use apmpr_sources::RegisteredSourceInspection;
use apmpr_state::{
...
fn open_store(root: &Path) -> Result<StateStore, String> {
    StateStore::open(root.join(".apmpr/state.sqlite3"))
        .map(|value| value.0)
        .map_err(|error| error.to_string())
}

pub fn list_sources(root: &Path) -> Result<Vec<RegisteredSourceInspection>, Box<dyn Error>> {
    let store = StateStore::open(root.join(".apmpr/state.sqlite3"))?.0;
    Ok(apmpr_sources::SourceManager::new(root).list(&store)?)
}
...
pub fn list_deployments(
    ...
    let reconciliation = apmpr_daemon::reconcile_project(root).ok();
...
fn definition_status(root: &Path, definition: &Path) -> DefinitionStatus {
    ...
    let validation_problems = apmpr_planner::plan_with_devices(&bundle, &devices)
        .err()
        .unwrap_or_default()
        .into_iter()
...
pub(crate) fn planning_devices(
    root: &Path,
) -> Result<BTreeMap<String, apmpr_planner::PlanningDevice>, String> {
    Ok(open_store(root)?
        .devices()
        .map_err(|error| error.to_string())?
        .into_iter()
...
fn load_memberships(root: &Path, deployment: &str, definition: &Path) -> Vec<MembershipRow> {
    let Ok(mut authored) = apmpr_planner::load_bundle(definition) else {
        return Vec::new();
    };
    let resolved_path = root
        ...
        .join(deployment)
        .join("resolved-deployment.yaml");
    if let Ok(resolved) = apmpr_planner::load_bundle(&resolved_path) {
    ...
    authored
        ...
        .filter_map(|instance| {
            ...
            let mut compatible_groups = authored
                ...
                .keys()
                .filter(|candidate| {
                    apmpr_planner::plan_with_membership_and_devices(
                        &authored,
                        &instance.name,
                        candidate,
                        &devices,
...
fn resource_rows(
    ...
    let containers = resources
        .iter()
        .filter(|resource| matches!(resource.kind, apmpr_state::ResourceKind::Container))
        .collect::<Vec<_>>();
