...
use apmpr_planner::{Block, Diagnostic};
use apmpr_state::StateStore;
...
fn build_draft(
    ...
    let bundle = apmpr_planner::load_bundle(definition)
    ...
    let draft_bundle = apmpr_planner::load_bundle_from_str(&draft, definition)
        .map_err(|error| CreateInstanceError::Definition(error.to_string()))?;
    ...
    let diagnostics = apmpr_planner::plan_with_devices(&draft_bundle, &devices)
        .err()
        .unwrap_or_default();
    Ok(DraftContext {
        preview: InstancePreview {
...
"#
        ...
        .unwrap();
        StateStore::open(root.join(".apmpr/state.sqlite3")).unwrap();
        path
    }
    ...
    fn imported_project() -> (TempDir, PathBuf, PathBuf) {
        ...
        fs::write(
            source.join("apmpr-profiles.yaml"),
            r#"version: 1
profiles:
  api:
    parameters:
...
"#,
    ...
    fn changed_and_not_imported_profiles_are_refused() {
        ...
        let store = StateStore::open(fresh.path().join(".apmpr/state.sqlite3"))
            .unwrap()
            .0;
        store
            .register_source(&RegisteredSource {
